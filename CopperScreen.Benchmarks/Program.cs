...
{
    Console.WriteLine("name\tagnus\tbackend\tframes/sec\treal-time\tms/frame\tallocated bytes\tcpu%\tagnus%\tdisplay%\tjit traces\tjit hits\tjit exits\tjit fallback\tjit invalid\tjit unsupported opcode\tjit unsupported ea\tjit host trap\tjit system\tjit exception\tjit generation\tjit boundary\tjit selfmod\tjit direct il\tjit helper il\tjit direct cpu\tjit direct mem\tjit spec helper\tjit v2 tier0\tjit v2 tier1\tjit v2 tier2\tjit v2 hits\tjit v2 exits\tjit v2 exit entry\tjit v2 exit branch\tjit v2 flags\tjit v2 writes\tjit v2 promotions\tjit v2 rejected\tjit v2 rej chip\tjit v2 rej host\tjit v2 rej dec\tjit v2 rej op\tjit v2 rej ea\tjit v2 rej budget\tjit v2 rej empty\tjit gen guard\tjit pure batch\tjit pure instr\tjit pure saved\tjit pure exits\tjit stopped ff\tjit stopped cycles\tlive events\tcopper\tpending\tfetches\tframebuffer\taudio\tdisplay summary\tdisk\tspecialization\tretained slots\tslot grants\tgrant mix\tdenied\tdenied mix\tblocked by\tlast denied\tdivergences\tlast divergence\tstatus");
}
...
static void WriteBenchmarkResult(BenchmarkRunResult result, BenchmarkOptions options)
{
    if (result.Missing)
    {
        Console.WriteLine($"{result.Workload.Name}\t{result.TimingMode}\t{result.CpuBackend}\tmissing{new string('\t', 68)}{result.StatusText}");
        if (options.InstructionMatrix)
        {
