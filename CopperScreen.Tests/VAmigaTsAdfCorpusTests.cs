...
public sealed class VAmigaTsAdfCorpusTests
{
	...
	private static string[] TraceCustomRegisterWrites(string relativePath, Machine machine)
	{
		...
		[
			...
			$"TRACE {relativePath}: rendered timeline row72 {string.Join("; ", machine.Bus.Display.RenderedCopperTimelineSegments.Where(segment => segment.Row == 72).Select(segment => $"{segment.XStart}-{segment.XStop} palette={segment.PaletteSnapshotIndex} color0=0x{segment.Color0:X3}"))}",
