...
namespace CopperMod.Amiga.CustomChips.Denise
{
    internal sealed partial class Display
    {
        private sealed class LiveDmaScratchContext
        {
            ...
            private readonly CopperBlitterWaitSnapshot _blitterWait;
            ...
            private readonly LiveLineState[] _lineStates;
            private readonly bool[] _lineStateDirtyFromScratchWrite;
            ...
            private bool _deniseDisplayWindowVerticallyOpen;
                ...
                OcsLiveDmaScratchCpuWrite pendingCpuWrite)
            {
                ...
                _lineStates = new LiveLineState[display._liveHardwareRowCount];
                _lineStateDirtyFromScratchWrite = new bool[display._liveHardwareRowCount];
                _frameStopCycle = display.GetLiveFrameStopCycle();
                _blitterWait = display.CaptureCopperBlitterWaitSnapshot();
                ...
                _physicalBitplanePipeline = display._physicalBitplanePipeline;
                ...
                _bplcon3 = display._bplcon3;
                _diwStart = display._diwStart;
                _diwStop = display._diwStop;
                _diwHigh = display._diwHigh;
                _diwHighValid = display._diwHighValid;
                _agnusDiwHigh = display._agnusDiwHigh;
                _agnusDiwHighValid = display._agnusDiwHighValid;
                _agnusDisplayWindow = display._agnusDisplayWindow;
                _deniseDisplayWindow = display._deniseDisplayWindow;
                _ddfStart = display._ddfStart;
                _ddfStop = display._ddfStop;
                _dataFetchWindow = display._dataFetchWindow;
                ...
                _deniseDisplayWindowVerticallyOpen = display._liveDeniseDisplayWindowVerticallyOpen;
                ...
                out OcsLiveDmaScratchResult result)
            {
                ...
                _requestRow = Math.Clamp(
                    _display.GetOutputRowForCycle(_display._liveFrameStartCycle, requestedCycle),
                    0,
                    _display._liveHardwareRowCount - 1);
                ...
                out int plane)
            {
                ...
                for (var candidateWord = 0; candidateWord < state.FetchWords; candidateWord++)
                {
                    ...
                    {
                        if (!TryGetBitplanePlaneForFetchSlot(candidateSlot, planeCount, state.FetchResolution, out var candidatePlane))
                        {
                            continue;
                        }
                        if (IsScratchBitplaneInputSuppressed(state, candidateWord, candidateSlot))
                        {
                            continue;
                        }

                        var fetchHorizontal = state.DataFetchStart + (candidateWord * state.FetchSlotStride) + candidateSlot;
                        var fetchCycle = AgnusChipSlotScheduler.AlignToSlot(state.LineStartCycle + ((long)fetchHorizontal * _display.CopperHpCycles));
                        if (fetchCycle < cycle)
                        {
                            continue;
                        }
            ...
            {
                var detailRow = Math.Clamp(
                    _display.GetOutputRowForCycle(_display._liveFrameStartCycle, requestedCycle),
                    0,
                    _display._liveHardwareRowCount - 1);
                var detailState = GetScratchLineState(detailRow);
                var detailPlan = _display._rowDmaPlans[GetRasterlineRingSlot(detailRow)];
                var nextPending = _pendingWriteIndex < _display._pendingWrites.Count
                    ? _display._pendingWrites[_pendingWriteIndex]
                    : default;
                return $"req={requestedCycle},captured={_capturedThroughCycle},copper={_copper.Cycle},pc=0x{_copper.Pc:X6},wait={_copper.Waiting},pm={_copper.PendingMove},ps={_copper.PendingSkip},pf={_copperFirstWordPending}," +
                    $"regs=0x{_bplcon0:X4}/0x{_bplcon3:X4}/0x{_dmacon:X4},diw=0x{_diwStart:X4}-0x{_diwStop:X4}/den:0x{_diwHigh:X4}:{_diwHighValid}/agn:0x{_agnusDiwHigh:X4}:{_agnusDiwHighValid}," +
                    $"dwin={_deniseDisplayWindow.HorizontalStart:X3}-{_deniseDisplayWindow.HorizontalStop:X3}:{_deniseDisplayWindow.VerticalStart:X3}-{_deniseDisplayWindow.VerticalStop:X3},awin={_agnusDisplayWindow.HorizontalStart:X3}-{_agnusDisplayWindow.HorizontalStop:X3}:{_agnusDisplayWindow.VerticalStart:X3}-{_agnusDisplayWindow.VerticalStop:X3}," +
                    $"ddf=0x{_ddfStart:X4}-0x{_ddfStop:X4}/eff:{_dataFetchWindow.Start:X3}-{_dataFetchWindow.Stop:X3}:{_dataFetchWindow.Resolution},pending={_pendingWriteIndex}/{_display._pendingWrites.Count}:{nextPending.Cycle}:0x{nextPending.Offset:X3}=0x{nextPending.Value:X4}," +
                    $"prep={_preparedFetchRow}:{_preparedFetchWord}:{_preparedFetchSlot},next={_nextFetchRow}:{_nextFetchWord}:{_nextFetchSlot},prepcy={GetNextPreparedBitplaneFetchCycle()},nextcy={GetNextBitplaneFetchCycle()},row={detailRow},line={detailState.LineStartCycle},df={detailState.DataFetchStart},fw={detailState.FetchWords},stride={detailState.FetchSlotStride},planes={detailState.PlaneCount},dmacon=0x{detailState.Dmacon:X4},ddf=0x{detailState.DdfStart:X4}-0x{detailState.DdfStop:X4},win={detailState.DisplayWindowVerticallyOpen},gen={detailState.Generation},plan={detailPlan.Valid}:{detailPlan.Generation}:{detailPlan.Row}:{detailPlan.BitplaneCount}:{detailPlan.Signature}:{ComputeRowDmaPlanSignature(detailState)}";
            }
            ...
            private ushort ReadScratchChipWordAtCycle(uint address, long cycle)
            {
                ...
                var value = _display._bus.ReadCurrentChipDmaWord(address);
            ...
            private bool AdvanceThrough(long cycle)
            {
                while (true)
                {
                    ...
                    if (nextCopper == next &&
                        (!_display.UsesPhysicalCopperPipeline ||
                         nextCopper < nextLineState && nextCopper < nextPreparedBitplane &&
                         nextCopper < nextBitplane && nextCopper < nextSprite))
                    {
                        if (!StepCopper(cycle))
                        {
                            return false;
                    ...
                    if (nextSprite == next && !ReserveSpriteFetch())
                    {
                        return false;
                    }
                }
            ...
            {
                if (_nextLineStateRow >= _display._liveHardwareRowCount)
                {
                    return long.MaxValue;
                }

                return _display.GetOutputRowStartCycle(_display._liveFrameStartCycle, _nextLineStateRow);
            }
            ...
            {
                if (_display.UsesPhysicalBitplanePipeline)
                {
                    return long.MaxValue;
                }
                ...
                var state = _display.GetLiveLineState(_preparedFetchRow);
                ...
                return AgnusChipSlotScheduler.AlignToSlot(state.LineStartCycle + ((long)fetchHorizontal * _display.CopperHpCycles));
            }

            private bool NormalizePreparedBitplaneCursor()
            {
                while (_preparedFetchRow < _display._liveHardwareRowCount)
                {
                    ...
                    while (_preparedFetchWord < state.FetchWords)
                    {
                        while (_preparedFetchSlot < state.FetchSlotStride)
                        {
                            if (TryGetBitplanePlaneForFetchSlot(
                                    ...
                                    out var plane) &&
                                !IsScratchBitplaneInputSuppressed(
                                    state,
                                    _preparedFetchWord,
                                    _preparedFetchSlot))
                            {
            ...
            private long GetNextBitplaneFetchCycle()
            {
                if (_display.UsesPhysicalBitplanePipeline)
                {
                    ...
                    if (!IsBitplaneDmaEnabled(_dmacon))
                    {
                        return long.MaxValue;
                    }
                }
            ...
            {
                while (_nextFetchRow < _display._liveHardwareRowCount)
                {
                    var state = GetScratchLineState(_nextFetchRow);
                    if (TryGetScratchRowDmaPlan(_nextFetchRow, state, out var plan))
                    {
            ...
            private long GetNextBitplaneOutputCycle()
            {
                var cycle = GetNextBitplaneFetchCycle();
                return !_display.UsesPhysicalBitplanePipeline || _physicalBitplanePipeline.HasOutput || cycle == long.MaxValue
                    ? cycle : checked(cycle + AgnusChipSlotScheduler.SlotCycles);
            }

            private bool StepPhysicalBitplane()
            {
                var cycle = GetNextBitplaneFetchCycle();
                if (_physicalBitplanePipeline.HasOutput)
                {
                    ...
                    if (issued.Granted)
                    {
                        ...
                        if (!_slots.MatchesAcceptedBitplaneWord(in access))
                        {
                            _unsupported = "bitplane-issued-output-lost";
                            return false;
                        }

                        _ = ReadScratchChipWordAtCycle(access.Request.Address, access.GrantedCycle);
                        _bitplaneFetches++;
                        RecordDma(cycle);
                        CompletePhysicalBitplanePointer(in issued);
                    }
                    if (_nextFetchRow == word.Row && _nextFetchWord == word.Word && _nextFetchSlot == word.Slot)
                    {
                        AdvanceBitplaneCursor();
                    }
                    return true;
                ...
                var state = GetScratchLineState(_nextFetchRow);
                var plane = _nextFetchPlane;
                var storageWord = _nextFetchWord + state.BitplaneWordIndexOffsets[plane];
                ...
                var valid = (state.PlaneHasRowMask & (1 << plane)) != 0;
                var address = valid
                    ? _display.AddDmaPointerOffset(state.BitplaneRowAddresses[plane], _nextFetchWord * 2)
                    : 0u;
                    ...
                    TryFindNextScratchRowDmaBitplaneEntry(plan, _nextFetchWord, _nextFetchSlot, out var entryIndex))
                {
                    ...
                    if (IsPhysicalBitplanePlanInput(
                            in plan, in entry, state, _nextFetchRow, plane, _nextFetchWord, _nextFetchSlot, cycle))
                    {
                        valid = entry.RowPresent;
                ...
                var lastWord = _nextFetchWord == state.FetchWords - 1;
                var wordMetadata = new BitplaneDmaWordMetadata(
                    _nextFetchRow, plane, _nextFetchWord, storageWord, _nextFetchSlot,
                    valid, lastWord, lastWord ? ((plane & 1) == 0 ? _bpl1mod : _bpl2mod) : (short)0)
                {
                    Plan = provenance
                };
                var accepted = default(AmigaBusAccessResult);
                var granted = valid && _slots.TryAcceptBitplaneInput(address, cycle, out accepted);
                if (!granted)
                {
                    var request = new AmigaBusAccessRequest(
                        ...
                        cycle, isWrite: false);
                    var outputCycle = checked(cycle + AgnusChipSlotScheduler.SlotCycles);
                    accepted = new AmigaBusAccessResult(request, outputCycle, outputCycle);
                }

                _physicalBitplanePipeline.Accept(in wordMetadata, in accepted, granted);
                return true;
            }

            private void RebaseFuturePhysicalBitplanePointer(int plane, long cycle)
            {
                _physicalBitplanePipeline.ObservePointerWrite(plane);
                var row = _display.GetOutputRowForCycle(_display._liveFrameStartCycle, cycle);
                if ((uint)row >= (uint)_display._liveHardwareRowCount)
                {
                    return;
                }

                var state = GetScratchLineState(row);
                var nextWord = _display.GetFirstBitplaneWordAfterCycle(state, cycle, plane);
            ...
            {
                var word = issued.Word;
                if (!word.LastWord && !issued.PointerWrittenAfterInput)
                {
                    return;
                }

                var pointer = _display.AddDmaPointerOffset(issued.Access.Request.Address, 2 + word.Modulo);
                _bitplanePointers[word.Plane] = pointer;
                _bitplaneBaseRows[word.Plane] = word.LastWord ? word.Row + 1 : word.Row;
                if (!word.LastWord)
                {
                    var state = GetScratchLineState(word.Row);
                    state.BitplanePointers[word.Plane] = pointer;
                    state.BitplaneBaseRows[word.Plane] = word.Row;
                    var nextWord = _display.GetFirstBitplaneWordAfterCycle(state, issued.OutputCycle, word.Plane);
                    state.BitplaneRowAddresses[word.Plane] = _display.AddDmaPointerOffset(pointer, -(nextWord * 2));
                    AdvanceRowDmaPlanVersion(state);
                }
            }
            ...
            private bool TryGetScratchRowDmaPlan(int row, LiveLineState state, out RowDmaPlan plan)
            {
                ...
                plan = _display._rowDmaPlans[GetRasterlineRingSlot(row)];
                if (plan.Valid &&
                    plan.Generation == _display._liveGeneration &&
                    plan.Row == row &&
                    (!_display.UsesPhysicalBitplanePipeline || plan.DmaPlanVersion == state.DmaPlanVersion) &&
                ...
                if (_display.UsesPhysicalBitplanePipeline)
                {
                    // A prior OUT carries its own frozen entry. Future inputs
                    // cannot borrow a live plan invalidated by a scratch write.
                    return false;
                }
            ...
            private long GetNextSpriteFetchCycle()
            {
                ...
                return _display.GetSpriteDmaFetchCycle(_display._liveFrameStartCycle, _nextSpriteRow, _nextSpriteIndex, _nextSpriteWord);
            }
            ...
            private bool ReserveSpriteFetch()
            {
                ...
                var slotCycle = _display.GetSpriteDmaFetchCycle(_display._liveFrameStartCycle, row, spriteIndex, word);
            ...
            private void AdvanceSpriteStateAfterFetch(int row, int spriteIndex, int word, uint address, long grantedCycle)
            {
                ...
                var value = ReadScratchChipWordAtCycle(address, grantedCycle);
                if (word == 0)
                {
            ...
            private bool StepPhysicalCopper(long targetCycle)
            {
                var cycle = _display.GetNextPhysicalCopperCycle(
                    _copper, IsLiveCopperDmaEnabled(), _copperListPointer, _slots, _blitterWait);
                if (cycle > targetCycle)
                {
                    return true;
                }
                if (_copper.PhysicalPipeline.HasIssuedWord &&
                    _copper.PhysicalPipeline.IssuedAccess.GrantedCycle == cycle)
                {
                    var access = _copper.PhysicalPipeline.IssuedAccess;
                    if (!_slots.MatchesAcceptedCopperWord(access))
                    {
                        _unsupported = "copper-issued-output-lost";
                        return false;
                    }
                    var value = ReadScratchChipWordAtCycle(access.Request.Address, access.GrantedCycle);
                    RecordDma(access.GrantedCycle);
                    if (_display.CompletePhysicalCopperWordState(
                            ref _copper, value, access, out var instruction))
                    {
                        var suppress = _copper.SuppressNextMove;
                        _copper.SuppressNextMove = false;
                        return ApplyCopperMove(instruction.MoveRegister, instruction.Second,
                            access.GrantedCycle, access.CompletedCycle, suppress);
                    }
                    return true;
                }
                if (_copper.PhysicalPipeline.IsVblankLatchDue(cycle))
                {
                    CompletePhysicalCopperVblankLatch(ref _copper, cycle, _copperListPointer);
                    return true;
                }

                var strobeDue = _copper.PhysicalPipeline.IsVblankStrobeDue(cycle);
                var wasStopped = _copper.Stopped;
                var dmaEnabled = IsLiveCopperDmaEnabled();
                if ((!strobeDue || _display.GetNextPhysicalCopperExecutionCycle(
                        _copper, dmaEnabled, _copperListPointer, _slots, _blitterWait) == cycle) &&
                    !StepPhysicalCopperInput(cycle))
                {
                    return false;
                }
                if (strobeDue)
                {
                    _display.BeginPhysicalCopperVblankRestart(
                        ref _copper, cycle, _copperListPointer, dmaEnabled, wasStopped);
                }
                return true;
            }

            private bool StepPhysicalCopperInput(long cycle)
            {
                if (_copper.PendingStart)
                {
                    _copper.StartFrom(_copperListPointer);
                }
                if (_copper.PhysicalPipeline.NeedsComparison &&
                    (_copper.WaitSecond & 0x8000) == 0 &&
                    !_blitterWait.CanPredictCompletion)
                {
                    _unsupported = "copper-blitter-wait";
                    return false;
                }

                // Consult this scratch state's future fixed cursors and copied
                // ledger. Do not query live ownership after a hypothetical write.
                var outputCycle = cycle + AgnusChipSlotScheduler.SlotCycles;
                var inputAvailable = _slots.CanAcceptCopperInputAt(cycle) &&
                    GetNextBitplaneOutputCycle() != outputCycle &&
                    GetNextSpriteFetchCycle() != outputCycle;
                var action = _display.AdvancePhysicalCopperControl(
                    ref _copper, cycle, inputAvailable, _slots, _blitterWait);
                ...
                {
                    var address = GetPhysicalCopperReadAddress(_copper, action);
                    if (!_slots.TryAcceptCopperInput(
                        address, GetPhysicalCopperRequestCycle(_copper, action, cycle), cycle, out var access))
                    {
                        _unsupported = "copper-input-availability-changed";
                        return false;
                    }
                    _copper.PhysicalPipeline.AcceptWord(action, cycle, access);
                    _copper.Cycle = access.GrantedCycle;
                }
                return true;
            }
            ...
            {
                var comparisonPending = _display.UsesPhysicalCopperPipeline
                    ? _copper.PhysicalPipeline.NeedsComparison
                    : _copper.Waiting || _copper.PendingSkip;
                var comparisonSecond = !_display.UsesPhysicalCopperPipeline && _copper.PendingSkip
                    ? _copper.PendingSkipSecond
                    : _copper.WaitSecond;
                if (comparisonPending && (comparisonSecond & 0x8000) == 0 &&
                    !_blitterWait.CanPredictCompletion &&
                    !_copper.Stopped && IsLiveCopperDmaEnabled())
                {
                    // A remote deadline must not make HasWorkThrough falsely
                    // report a supported CPU grant across an unresolved F.
                    _unsupported = "copper-blitter-wait";
                    return long.MaxValue;
                }

                if (_display.UsesPhysicalCopperPipeline)
                {
                    return _display.GetNextPhysicalCopperCycle(
                        _copper, IsLiveCopperDmaEnabled(), _copperListPointer, _slots, _blitterWait);
                }
                if (_copperFirstWordPending)
                {
                    return _display.GetCopperSecondWordRequestCycle(_copperFirstWordAccess);
                }
                ...
                if (_copper.Pc == 0 && _copperListPointer == 0)
                {
                    return long.MaxValue;
                }

                if (_copper.RestartArmed || _copper.ReadyToRequest)
                {
                    return _copper.Cycle;
                }

                if (_copper.Waiting)
                {
                    var blitterReadyCycle = _display.GetCopperBlitterReadyCycle(
                        _copper.WaitSecond, _copper.Cycle, _blitterWait);
                    if (blitterReadyCycle > targetCycle)
                    {
                        return long.MaxValue;
                    }
                    if (!_display.TryGetCopperWaitCycle(
                            ...
                            Math.Max(_copper.Cycle, blitterReadyCycle),
                            targetCycle + 1,
            ...
            private bool StepCopper(long targetCycle)
            {
                if (_display.UsesPhysicalCopperPipeline)
                {
                    return StepPhysicalCopper(targetCycle);
                }
                if (_copperFirstWordPending)
                {
                    ...
                    RecordCopperBitplaneRgaCollision(secondRequestCycle);
                    var preserveSecondWordPhysicalPhase =
                        _display._bus.GetBeamPosition(_copperFirstWordAccess.GrantedCycle).BeamLine !=
                        _display._bus.GetBeamPosition(secondRequestCycle).BeamLine;
                    var secondAccess = _slots.ReserveCopperDmaSlot(
                        secondAddress,
                        secondRequestCycle,
                        preserveSecondWordPhysicalPhase);
                    var second = ReadScratchChipWordAtCycle(secondAddress, secondAccess.GrantedCycle);
                ...
                if (_copper.PendingSkip)
                {
                    ...
                    if (_display.IsCopperComparisonSatisfied(
                            first,
                            second,
                            _display._liveFrameStartCycle,
                            skipCycle,
                            _display.IsCopperBlitterFinishedForWait(second, skipCycle, _blitterWait)))
                    {
                ...
                if (_copper.RestartArmed)
                {
                    if (_copper.Cycle > targetCycle)
                    {
                        return true;
                    }

                    _copper.AdvanceWaitRestartArm(
                        _copper.Cycle,
                        _display.GetAgnusBitplaneFetchPlaneCount());
                    return true;
                }

                if (_copper.ReadyToRequest)
                {
                    _copper.AdvanceWaitRestartStage(_copper.Cycle);
                }
                ...
                if (_copper.Waiting)
                {
                    ...
                    if (blitterReadyCycle > _copper.Cycle)
                    {
                        ...
                        if (!_display.IsCopperBlitterFinishedForWait(
                                _copper.WaitSecond, _copper.Cycle, _blitterWait))
                        {
                            _unsupported = "copper-blitter-wait";
                            return false;
                    ...
                    var comparisonStartCycle = bfdReleasedByTermination
                        ? waitCycle
                        : _copper.Cycle;
                    if (_copper.SatisfiedWaitRunCount == 0)
                    {
                        _copper.WaitRunBeganWithBlockingComparison =
                    ...
                    var beamSatisfiedAtBlitterTermination =
                        ...
                        _display.IsCopperComparisonSatisfied(
                            ...
                            _copper.WaitSecond,
                            _display._liveFrameStartCycle,
                            _blitterWait.LastTerminationCycle,
                            blitterFinished: true);
                    if (bfdReleasedByTermination &&
                        beamSatisfiedAtBlitterTermination &&
                        _display.GetCopperHorizontalForCycle(
                            _display._liveFrameStartCycle,
                            _blitterWait.LastTerminationCycle) >= DefaultDdfStart)
                    {
                    ...
                    if (readyAtWakeCycle && !bfdReleasedByTermination)
                    {
                        if (_blitterWait.BusPipelineActive &&
                            _blitterWait.NastyPriorityEnabled)
                        {
                            resumeCycle += AgnusChipSlotScheduler.SlotCycles;
                        }
                    }
                    ...
                    if (!bfdReleasedByTermination &&
                        (_copper.WaitFirst & 0x00FE) <= 2)
                    {
                        _copper.PreserveFirstWordPhysicalPhase = true;
                    }
                    var fourPlaneWait =
                    ...
                    var fourPlaneTailRunPhaseReusable =
                        _display.IsFourPlaneWaitTailRunPhaseReusable(
                            _copper.WaitFirst);
                    if (fourPlaneTailRunPhaseReusable &&
                        waitCycle > comparisonStartCycle &&
                        _copper.WaitRunContinuesFromDifferentInstruction &&
                        _copper.WaitRunCrossedIntoLineTail)
                    {
                        _copper.WaitRunCrossedIntoLineTailAfterBlockingComparison = true;
                    ...
                    var fourPlaneWaitRunContinuation =
                        ...
                        waitCycle <= comparisonStartCycle &&
                        (_copper.SatisfiedWaitRunCount > 0 ||
                         _copper.WaitRunContinuesFromDifferentInstruction);
                    var fourPlaneWaitRunReusesControlPhase =
                        fourPlaneWaitRunContinuation &&
                        (!_copper.WaitRunContinuesFromDifferentInstruction ||
                         ((_copper.WaitFirst & 0x00FE) -
                          (_copper.PreviousWaitRunFirst & 0x00FE)) <= 12);
                    var fourPlaneRunControlBlocked =
                        _copper.WaitRunControlBlocked ||
                        (_copper.WaitRunContinuesFromDifferentInstruction &&
                         _copper.PreviousWaitRunControlBlocked);
                    if (fourPlaneWaitRunContinuation &&
                        (fourPlaneRunControlBlocked ||
                         (_copper.WaitRunContinuesFromDifferentInstruction &&
                          (_copper.PreviousWaitRunFirst & 0xFF00) ==
                          (_copper.WaitFirst & 0xFF00) &&
                          ((_copper.PreviousWaitRunFirst & 0x0002) == 0 ||
                           (_copper.WaitFirst & 0x00F0) == 0x00B0))))
                    {
                        resumeCycle -= AgnusChipSlotScheduler.SlotCycles;
                    }
                    ...
                    {
                        var incomingRgaBlocked = _display.IsBitplaneRgaIncomingPhase(waitCycle);
                        var overlapsAdjacentRgaStage =
                            _display.IsBitplaneRgaDecisionPhase(waitCycle) ||
                    ...
                    else if (_copper.WaitStartCarryPending)
                    {
                        if (_copper.WaitStartCarrySkipCount > 0)
                        {
                            _copper.WaitStartCarrySkipCount--;
                            if (!_display.IsCopperWaitRestartAtPhysicalLineTail(resumeCycle))
                            {
                                _copper.PendingWaitStartTail = true;
                            }
                        }
                        else
                        {
                            ...
                            if (_copper.WaitStartCarryAdjacent)
                            {
                                _copper.WaitInheritedAdjacentControlPhase = true;
                                _copper.WaitInheritedAdjacentFirst = _copper.WaitFirst;
                                _copper.WaitInheritedAdjacentSecond = _copper.WaitSecond;
                                inheritedAdjacentControlPhaseCreated = true;
                            }
                            _copper.WaitStartCarryAdjacent = false;
                        }
                    ...
                    {
                        _copper.WaitStartCarryPending = false;
                        _copper.WaitStartCarrySkipCount = 0;
                    }
                    if (fourPlaneWaitRunContinuation)
                    {
                        ...
                        _copper.WaitStartCarryPending = false;
                        _copper.WaitStartCarryAdjacent = false;
                        _copper.WaitRunControlBlocked = fourPlaneRunControlBlocked;
                    }
                    _copper.WaitRunControlBlocked |= restartIncomingRgaBlocked;
                    if (_copper.SatisfiedWaitRunCount > 0 &&
                        !_copper.WaitRunControlBlocked &&
                    ...
                    var reuseInheritedAdjacentControlPhase =
                        ...
                        _copper.WaitInheritedAdjacentControlPhase &&
                        _copper.WaitInheritedAdjacentFirst == _copper.WaitFirst &&
                        _copper.WaitInheritedAdjacentSecond == _copper.WaitSecond;
                    var restartAfterDataFetchStop =
                        IsCopperWaitRestartAfterDataFetchStop(resumeCycle);
                    var reuseExhaustedAdjacentCarryPhase =
                        _copper.WaitStartCarryPending &&
                        _copper.WaitStartCarryAdjacent &&
                        _copper.WaitStartCarrySkipCount == 0 &&
                        (restartAfterDataFetchStop ||
                         (_copper.SatisfiedWaitRunCount >= 2 &&
                          !_display.IsCopperWaitRestartAtPhysicalLineTail(resumeCycle) &&
                          !_display.DidCopperWaitRestartCrossPhysicalLine(comparisonStartCycle, resumeCycle)));
                    if (!inheritedAdjacentControlPhaseCreated)
                    {
                        _copper.WaitInheritedAdjacentControlPhase = false;
                    }
                    ...
                    var projectCrossedLineTailRun =
                        ...
                        _copper.WaitRunCrossedIntoLineTail &&
                        (_copper.WaitFirst & 0x00FE) == 0x00C4;
                    if (reuseRunControlPhase &&
                        _copper.WaitRunCrossedIntoLineTailAfterBlockingComparison &&
                    ...
                    var waitTailPresentationX =
                        projectCrossedLineTailRun
                            ? Math.Max(
                                0,
                                _display.GetCopperWaitTailPresentationX(_copper.WaitFirst) -
                                (_copper.SatisfiedWaitRunCount * 8))
                            : presentNextMoveFromReusedWaitTail ||
                              _display.GetAgnusBitplaneFetchPlaneCount(_bplcon0) != 4
                            ? -1
                            : _display.GetCopperWaitTailPresentationX(_copper.WaitFirst);
            ...
            private bool ApplyLoadedCopperInstruction(CopperInstructionLatch instruction, long targetCycle)
            {
                ...
                if (_display.IsCopperComparisonSatisfied(
                        ...
                        _display.IsCopperBlitterFinishedForWait(
                            instruction.Second, instruction.ControlStopCycle, _blitterWait)))
                {
                    _copper.SuppressNextMove = true;
                }
            ...
            private void BeginCopperInstruction(uint pc, long fetchCycle)
            {
                ...
                RecordCopperBitplaneRgaCollision(fetchCycle);
                _copperFirstWordAccess = _slots.ReserveCopperDmaSlot(
                    pc,
                    fetchCycle,
                    _copper.PreserveFirstWordPhysicalPhase);
                _copper.PreserveFirstWordPhysicalPhase = false;
            ...
            {
                var row = _display.GetOutputRowForCycle(_display._liveFrameStartCycle, copperRequestCycle);
                if ((uint)row >= (uint)_display._liveHardwareRowCount)
                {
                    return;
                }
                ...
                if (_display.TryMapCopperRequestToLateBitplaneCollision(
                        state,
                        copperRequestCycle,
                        out var plane,
                ...
                bool suppressMove)
            {
                ...
                _copper.PresentNextMoveFromReusedWaitTail = false;
            ...
            private bool ApplyScratchCopperMove(ushort register, ushort value, long cycle)
            {
                ...
                if (register == 0x096)
                {
                    ...
                    var copperDmaWasEnabled = IsLiveCopperDmaEnabled();
                    _dmacon = ApplySetClearPreview(_dmacon, value);
                    if (!copperDmaWasEnabled && IsLiveCopperDmaEnabled() && _copper.Cycle < cycle)
                    {
                        _copper.Cycle = cycle;
                    }
                    ...
                    if (!bitplaneDmaWasEnabled && IsBitplaneDmaEnabled(_dmacon))
                    {
                        SetScratchBitplaneBaseRows(
                            0,
                            _display.GetAgnusBitplaneFetchPlaneCount(_bplcon0),
                ...
                {
                    PreserveStartedBitplaneRow(cycle);
                    var oldPlaneCount = _display.GetAgnusBitplaneFetchPlaneCount(_bplcon0);
                    var newPlaneCount = _display.GetAgnusBitplaneFetchPlaneCount(value);
                    var bitplaneCursorCutoffCycle =
                        _display.GetBitplaneCursorCutoffAfterPlaneCountChange(
                            cycle,
                            oldPlaneCount,
                            newPlaneCount);
                    ...
                    if (oldPlaneCount > 0 && newPlaneCount == 0)
                    {
                        _display.CommitLateBitplaneRgaCollisionsForPreviousRow(cycle);
                    }

                    _bplcon0 = value;
                    RefreshScratchDisplayGeometry();
                    ...
                    ResetBitplaneCursorsFromCycle(cycle);
                    if (bitplaneCursorCutoffCycle != cycle)
                    {
                        SeekBitplaneCursorsAfterCycle(bitplaneCursorCutoffCycle);
                    }
                    return true;
                }
                ...
                if (register == 0x080)
                {
                    ...
                    RebasePendingCopperStartAfterPointerWrite(previousPointer, cycle);
                    return true;
                }
                ...
                if (register >= 0x110 && register <= 0x11E)
                {
                ...
                {
                    _copper.PendingWaitTailPresentationX = -1;
                    return true;
                }

                if (register >= 0x0E0 && register <= 0x0FE)
                {
                    ...
                    if ((uint)plane < (uint)_bitplanePointers.Length)
                    {
                        ...
                        if (_display.UsesPhysicalBitplanePipeline)
                        {
                            RebaseFuturePhysicalBitplanePointer(plane, cycle);
                        }
                    }
                ...
                {
                    if (IsBitplaneDmaEnabled(_dmacon) && _display.GetAgnusBitplaneFetchPlaneCount(_bplcon0) > 0)
                    {
                ...
                var impact = CustomRegisterScheduleClassifier.GetPotentialImpact(
                    _display._chipset,
                    register);
                if (HasCopperHardwareSideEffect(register) ||
                    CustomRegisterScheduleClassifier.AffectsEventSchedule(impact))
                {
            ...
            private void RebasePendingCopperStartAfterPointerWrite(uint previousPointer, long cycle)
            {
                if (_copper.PendingStart &&
                    previousPointer == 0 &&
            ...
            private int GetSpriteControlRow(long cycle)
                => Math.Clamp(_display.GetOutputRowForCycle(_display._liveFrameStartCycle, cycle), 0, LowResOutputHeight);

            private LiveLineState GetScratchLineState(int row)
            {
                row = Math.Clamp(row, 0, _display._liveHardwareRowCount - 1);
            ...
            private void CaptureScratchLineState(int row, LiveLineState state)
            {
                ...
                {
                    CopyScratchLineState(_display.GetLiveLineState(row), state);
                    return;
                }
            ...
            private void CaptureScratchLineStateFromCurrentRegisters(int row, LiveLineState state)
            {
                ...
                state.LineStartCycle = _display.GetOutputRowStartCycle(_display._liveFrameStartCycle, row);
                state.DisplayWindowVerticallyOpen = _displayWindowVerticallyOpen;
                state.DeniseDisplayWindowVerticallyOpen = _deniseDisplayWindowVerticallyOpen;
                state.Resolution = _display.GetDeniseResolution(_bplcon0);
                ...
                state.PlaneCount = _display.GetAgnusBitplaneFetchPlaneCount(_bplcon0);
                state.DecodePlaneCount = _display.GetDeniseBitplaneDecodePlaneCount(_bplcon0);
                ...
                Array.Clear(state.BitplaneWordIndexOffsets);
                for (var plane = 0; plane < LiveBitplanePlaneCount; plane++)
                {
                    if (_display.IsLatchedOnlyOcsBpu7Plane(state.Bplcon0, plane))
                    {
            ...
            private static void CopyScratchLineState(LiveLineState source, LiveLineState destination)
            {
                ...
                destination.DmaPlanVersion = source.DmaPlanVersion;
                ...
                destination.DeniseDisplayWindowVerticallyOpen = source.DeniseDisplayWindowVerticallyOpen;
                ...
                destination.BitplaneInputSuppressedThroughCycle =
                    source.BitplaneInputSuppressedThroughCycle;
                ...
                Array.Copy(
                    source.BitplaneWordIndexOffsets,
                    destination.BitplaneWordIndexOffsets,
                    source.BitplaneWordIndexOffsets.Length);
            ...
            private void ResetBitplaneCursorsFromCycle(long cycle)
            {
                ...
                var invalidateRow = currentRowStarted
                    ? Math.Min(row + 1, _display._liveHardwareRowCount)
                    : row;
            ...
            private void SeekBitplaneCursorsAfterCycle(long cycle)
            {
                var firstEligibleCycle = checked(cycle + AgnusChipSlotScheduler.SlotCycles);
                var row = Math.Clamp(
                    _display.GetOutputRowForCycle(_display._liveFrameStartCycle, firstEligibleCycle),
                    0,
                    _display._liveHardwareRowCount - 1);
                var state = GetScratchLineState(row);
                ...
                if (!TryGetBitplaneCursorAtOrAfterCycle(
                        ...
                        out var plane))
                {
                    row = Math.Min(row + 1, _display._liveHardwareRowCount);
                    word = 0;
                    slot = 0;
                    plane = 0;
                }
            ...
            private bool HasStartedBitplaneFetch(LiveLineState state, long cycle)
            {
                ...
                for (var word = 0; word < state.FetchWords; word++)
                {
                    for (var slot = 0; slot < state.FetchSlotStride; slot++)
                    {
                        if (!TryGetBitplanePlaneForFetchSlot(
                                slot,
                                Math.Max(0, state.PlaneCount),
                                state.FetchResolution,
                                out _) ||
                            IsScratchBitplaneInputSuppressed(state, word, slot))
                        {
                            continue;
                        }
                        ...
                        var fetchCycle = AgnusChipSlotScheduler.AlignToSlot(
                            ...
                            ((long)fetchHorizontal * _display.CopperHpCycles));
            ...
            private bool IsScratchBitplaneInputSuppressed(
                LiveLineState state,
                int word,
                int slot)
            {
                ...
                var fetchCycle = AgnusChipSlotScheduler.AlignToSlot(
                    state.LineStartCycle +
                    ((long)fetchHorizontal * _display.CopperHpCycles));
                return fetchCycle <= state.BitplaneInputSuppressedThroughCycle;
            }

            private void AdvanceDisplayWindowStateToLine(int targetLine)
            {
                targetLine = Math.Clamp(targetLine, 0, _display._timing.LongFrameLines - 1);
                while (_displayWindowStateLine <= targetLine)
                {
                    if (_displayWindowStateLine == _agnusDisplayWindow.VerticalStop)
                    {
                        _displayWindowVerticallyOpen = false;
                    }

                    if (_displayWindowStateLine == _agnusDisplayWindow.VerticalStart)
                    {
                        _displayWindowVerticallyOpen = true;
                    }

                    if (_displayWindowStateLine == _deniseDisplayWindow.VerticalStop)
                    {
                        _deniseDisplayWindowVerticallyOpen = false;
                    }

                    if (_displayWindowStateLine == _deniseDisplayWindow.VerticalStart)
                    {
                        _deniseDisplayWindowVerticallyOpen = true;
                    }
            ...
            private bool IsCopperWaitRestartAfterDataFetchStop(long cycle)
                => _display._bus.GetBeamPosition(cycle).BeamHorizontal > GetDataFetchStopValue();
