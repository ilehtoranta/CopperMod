...
public sealed class SdlGameControllerDbTests
{
	...
	public void Parser_ReadsGuidPlatformButtonsAxesHatsAndHalfAxes()
	{
		...
		Assert.Contains(mapping.Bindings, binding => binding.Target == ControllerElement.A && binding.Source.Kind == SdlInputSourceKind.Button);
		Assert.Contains(mapping.Bindings, binding => binding.Target == ControllerElement.DPadUp && binding.Source.Kind == SdlInputSourceKind.Hat);
		Assert.Contains(mapping.Bindings, binding => binding.Target == ControllerElement.LeftTrigger && binding.Source.Polarity == SdlPolarity.Positive);
		Assert.Contains(mapping.Bindings, binding => binding.Target == ControllerElement.RightTrigger && binding.Source.Polarity == SdlPolarity.Negative);
		Assert.Contains(mapping.Bindings, binding => binding.Target == ControllerElement.LeftStickY && binding.Source.Invert);
	}
