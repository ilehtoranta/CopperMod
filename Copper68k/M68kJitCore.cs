...
namespace Copper68k
{
    ...
    internal sealed class M68kJitOptions
    {
        public M68kJitOptions(
            ...
            bool enableV2,
            bool enableV2Tier3,
            bool enableV2MemoryRead,
            bool enableV2BusAccess,
            bool enableV2FastRead,
            bool enableV2BusGraph,
            bool enableAsyncJit,
            ...
            M68040Move16CopyStrategy move16CopyStrategy = M68040Move16CopyStrategy.Auto)
        {
            CpuModel = cpuModel;
            EnableV2 = enableV2;
            EnableV2Tier3 = enableV2Tier3;
            EnableV2MemoryRead = enableV2MemoryRead;
            EnableV2BusAccess = enableV2BusAccess;
            EnableV2FastRead = enableV2FastRead;
            EnableV2BusGraph = enableV2BusGraph;
            EnableAsyncJit = enableAsyncJit;
        ...
        public bool EnableV2 { get; }

        public bool EnableV2Tier3 { get; }

        public bool EnableV2MemoryRead { get; }

        public bool EnableV2BusAccess { get; }

        public bool EnableV2FastRead { get; }

        public bool EnableV2BusGraph { get; }

        public bool EnableAsyncJit { get; }

        public bool EnableAsyncJitSyncFallback { get; }
    ...
    internal sealed class M68kJitCore : IM68kBatchCore, IM68kInstructionFrequencyProvider, IM68000InterruptRecognition, IM68000PrefetchDiagnostics
    {
        ...
        private static readonly MethodInfo ExecuteM68040FallbackForV2BatchMethod =
            typeof(M68kJitCore).GetMethod(
                nameof(ExecuteCompiledM68040Fallback),
                BindingFlags.Instance | BindingFlags.NonPublic) ??
            throw new MissingMethodException(typeof(M68kJitCore).FullName, nameof(ExecuteCompiledM68040Fallback));
        ...
        private bool _compiledM68040FpuExceptionPending;
        ...
        internal static M68kJitCore CreateM68040(IM68kBus bus)
            => new M68kJitCore(
                ...
                new M68kJitOptions(
                    ...
                    enableV2: true,
                    enableV2Tier3: true,
                    enableV2MemoryRead: true,
                    enableV2BusAccess: true,
                    enableV2FastRead: true,
                    enableV2BusGraph: true,
                    enableAsyncJit: true,
                    enableAsyncJitSyncFallback: true,
        ...
        private M68kJitCore(IM68kBus bus, M68kJitOptions options)
        {
            ...
            _v2Enabled = options.EnableV2;
            _v2Tier3Enabled = options.EnableV2Tier3;
            _v2MemoryReadEnabled = options.EnableV2MemoryRead;
            _v2BusAccessEnabled = options.EnableV2BusAccess;
            _v2FastReadEnabled = options.EnableV2FastRead;
        ...
        private static IM68kCore CreateFallback(
            IM68kBus bus,
            M68kCpuState state,
            M68kInstructionFrequencyMatrix instructionFrequency,
            M68kJitCpuModel cpuModel)
        ...
        public int ExecuteInstructions(int maxInstructions, long? targetCycle, IM68kInstructionBoundary boundary)
        {
                ...
                (!targetCycle.HasValue || State.Cycles < targetCycle.Value))
            {
                ...
                {
                    if (!HandleM68040CompiledMmuFault(ex.Fault, boundary))
                    {
                        break;
                    }
        ...
        private bool TryGetPhysicalInstructionAddress(uint logicalAddress, out uint physicalAddress)
        {
            if (_cpuModel != M68kJitCpuModel.M68040)
            {
                physicalAddress = Normalize(logicalAddress);
                return true;
            }
            ...
            if (TryTranslateM68040Address(
                    ...
                    out _))
            {
                physicalAddress = Normalize(physicalAddress);
                return true;
            }
        ...
        private bool ShouldFlushM68040JitAfterFallback(uint instructionPc, ushort opcode)
        {
            if (_cpuModel != M68kJitCpuModel.M68040)
            {
                return false;
            ...
            if (opcode != 0x4E7B ||
                !TryReadInstructionWord(instructionPc + 2, out var extension))
            {
                return false;
            }

            return IsM68040JitInvalidatingControlRegister(extension & 0x0FFF);
        }

        private static bool IsM68040JitInvalidatingControlRegister(int register)
            => register is 0x002 or 0x003 or 0x004 or 0x005 or 0x006 or 0x007 or 0x802 or 0x806 or 0x807;

        private bool TryReadInstructionWord(uint logicalAddress, out ushort value)
        {
            value = 0;
            if (!TryTranslateM68040Address(
                logicalAddress,
            ...
            try
            {
                if (_bus is IM68kCodeReader codeReader)
                {
                    value = codeReader.ReadHostWord(physicalAddress);
                    return true;
                }
        ...
        private IM68kCodeReader CreateTraceCodeReader()
            => _cpuModel == M68kJitCpuModel.M68040 ? new M68040JitCodeReader(this) : _amigaBus!;

        private bool IsJitEligibleInstructionAddress(uint logicalAddress)
        {
            if (_amigaBus == null)
            {
                return false;
        ...
        private bool CanQueueAsyncTraceCompilation(uint logicalRoot)
        {
            if (_cpuModel != M68kJitCpuModel.M68040)
            {
                return true;
            ...
            if (TryReadInstructionWord(logicalRoot, out var opcode) &&
        ...
        {
            private readonly M68kJitCore _owner;
            ...
            public ushort ReadHostWord(uint address)
            {
                if (_owner.TryReadInstructionWord(address, out var value))
                {
                    return value;
                }
        ...
        private void ObserveHotRoot(uint pc)
        {
            ...
            if (_asyncJitEnabled && CanQueueAsyncTraceCompilation(pc))
            {
                if (TryQueueAsyncTraceCompilation(
                    pc,
                    V2TraceTier.Tier0,
        ...
        {
            plan = default;
            if (_amigaBus == null || !IsJitEligibleInstructionAddress(root))
            {
                return false;
            }
            ...
            while (count < MaxTraceInstructions && byteCount < MaxTraceBytes)
            {
                ...
                if (!M68kDecoder.TryDecode(codeReader, pc, out var instruction, out var decodeReason, _cpuModel))
                {
        ...
        private void RecordTracePlanCompiledCounters(M68kTraceCompilationPlan plan)
        {
            ...
            if (!plan.V2Trace.IsEmpty)
            {
                ...
                foreach (var instruction in plan.V2Trace.Instructions)
                {
                    if (instruction.Operation is
                        M68kJitOperation.M68040Fbcc or
                        M68kJitOperation.M68040Fdbcc or
                        M68kJitOperation.M68040Fscc or
                        M68kJitOperation.M68040Ftrapcc)
                    {
                        _counters.M68040NativeFpuConditionSites++;
                    }
                }
            ...
            while (work.Count > 0 && collected.Count < maxInstructions)
            {
                ...
                while (collected.Count < maxInstructions)
                {
                    ...
                    if (IsV2BranchInstruction(instruction))
                    {
                        ...
                        if (instruction.Operation is M68kJitOperation.Bcc or M68kJitOperation.Dbcc or
                            M68kJitOperation.M68040Fbcc or M68kJitOperation.M68040Fdbcc)
                        {
        ...
        private ushort[] CaptureTraceWords(uint root, int byteLength)
        {
            ...
            for (var i = 0; i < words.Length; i++)
            {
                if (_cpuModel == M68kJitCpuModel.M68040)
                {
                    if (!TryReadInstructionWord(address, out words[i]))
                    {
                        return Array.Empty<ushort>();
                    }
                }
        ...
        private static V2TraceCompilation CompileV2FromPlan(
            uint root,
            V2TracePlan plan,
            bool useGraph,
        ...
        private bool IsV2Instruction(M68kDecodedInstruction instruction, out V2TraceRejectionReason rejectionReason)
        {
            ...
            var supported = instruction.Operation switch
            {
                ...
                M68kJitOperation.M68040Fbcc or M68kJitOperation.M68040Fdbcc or
                M68kJitOperation.M68040Fscc or M68kJitOperation.M68040Ftrapcc => _cpuModel == M68kJitCpuModel.M68040,
                _ => false
            };
            if (supported)
            {
            ...
            out V2TraceRejectionReason rejectionReason)
        {
            ...
            var supported = instruction.Operation switch
            {
                ...
                M68kJitOperation.M68040Fbcc or M68kJitOperation.M68040Fdbcc or
                M68kJitOperation.M68040Fscc or M68kJitOperation.M68040Ftrapcc => options.CpuModel == M68kJitCpuModel.M68040,
                _ => false
            };
            if (supported)
            {
        ...
        private static bool IsV2BranchInstruction(M68kDecodedInstruction instruction)
            => instruction.Operation is M68kJitOperation.Bra or M68kJitOperation.Bcc or M68kJitOperation.Dbcc or
                M68kJitOperation.M68040Fbcc or M68kJitOperation.M68040Fdbcc or
                M68kJitOperation.Jmp;
            ...
            ref int dirtyAddressRegisters)
        {
            ...
            switch (instruction.Operation)
            {
                ...
                case M68kJitOperation.BitDynamic:
                    ...
                    return;
                case M68kJitOperation.Dbcc:
                case M68kJitOperation.M68040Fdbcc:
                    MarkV2DataRegisterDirty(instruction.Register, ref dirtyDataRegisters);
                    return;
                case M68kJitOperation.M68040Fscc:
                    MarkV2DataRegisterDirty(instruction.Register, ref dirtyDataRegisters);
                    return;
            }
        }
            ...
            bool recordOutOfBlockSideExit)
        {
            switch (instruction.Operation)
            {
                ...
                case M68kJitOperation.M68040Fpu:
                    ...
                    return;
                case M68kJitOperation.M68040Fbcc:
                    EmitV2M68040Fbcc(il, context, instruction, labels, addressToIndex, exit, root, codeStart, byteLength, recordOutOfBlockSideExit);
                    return;
                case M68kJitOperation.M68040Fdbcc:
                    EmitV2M68040Fdbcc(il, context, instruction, labels, addressToIndex, exit, root, codeStart, byteLength, recordOutOfBlockSideExit);
                    return;
                case M68kJitOperation.M68040Fscc:
                    EmitV2M68040Fscc(il, context, instruction, exit);
                    return;
                case M68kJitOperation.M68040Ftrapcc:
                    EmitV2M68040Ftrapcc(il, context, instruction, exit);
                    return;
            ...
            bool add)
        {
            ...
            if (instruction.Variant == 1)
            {
                ...
                context.EmitAddCycles(GetAluDataToEaCyclesForTiming(instruction.Destination, instruction.Size));
                return;
            }
            ...
            context.EmitAddCycles(GetAluEaToDataCyclesForTiming(instruction.Source, instruction.Size));
        }
        ...
        private static void EmitV2Tst(ILGenerator il, V2EmitContext context, M68kDecodedInstruction instruction)
        {
            ...
            il.Emit(OpCodes.Stloc, value);
            context.EmitSetPendingLogic(value, instruction.Size);
            context.EmitAddCycles(GetUnaryCyclesForTiming(M68kJitOperation.Tst, instruction.Destination, instruction.Size));
        }
        ...
        {
            return GetUnaryCyclesForTiming(M68kJitOperation.Clr, instruction.Destination, instruction.Size);
        }
        ...
        private static void EmitV2Compare(ILGenerator il, V2EmitContext context, M68kDecodedInstruction instruction)
        {
            ...
            context.EmitAddCycles(GetCompareCyclesForTiming(instruction.Source, instruction.Size));
        }
        ...
        private static void EmitV2CompareImmediate(ILGenerator il, V2EmitContext context, M68kDecodedInstruction instruction)
        {
            ...
            EmitV2ArithmeticResult(il, destination, source, result, instruction.Size, add: false);
            context.EmitSetPendingArithmetic(V2PendingFlags.Subtract, destination, source, result, instruction.Size, setExtend: false);
            context.EmitAddCycles(GetCmpiCyclesForTiming(instruction.Destination, instruction.Size));
        }
        ...
        private static void EmitV2ImmediateLogical(ILGenerator il, V2EmitContext context, M68kDecodedInstruction instruction, int operation)
        {
            if (instruction.Destination.Kind != M68kJitEaKind.DataRegister)
            {
                ...
                context.EmitAddCycles(GetImmediateAluCyclesForTiming(instruction.Destination, instruction.Size));
                return;
            }
        ...
        private static void EmitV2Not(ILGenerator il, V2EmitContext context, M68kDecodedInstruction instruction)
        {
            ...
            context.EmitStoreDataRegister(instruction.Destination.Register, value, instruction.Size);
            context.EmitSetPendingLogic(value, instruction.Size);
            context.EmitAddCycles(GetUnaryCyclesForTiming(M68kJitOperation.Not, instruction.Destination, instruction.Size));
        }
        ...
        private static void EmitV2Neg(ILGenerator il, V2EmitContext context, M68kDecodedInstruction instruction)
        {
            ...
            context.EmitAddCycles(GetUnaryCyclesForTiming(M68kJitOperation.Neg, instruction.Destination, instruction.Size));
        }
        ...
        private static void EmitV2Negx(ILGenerator il, V2EmitContext context, M68kDecodedInstruction instruction)
        {
            ...
            context.EmitAddCycles(GetUnaryCyclesForTiming(M68kJitOperation.Negx, instruction.Destination, instruction.Size));
        }
            ...
            Label exit)
        {
            ...
            {
                var fastMethod = M68kOperationEmitter.GetM68040FpuFastRegisterMethod(instruction.Condition);
                var exceptionalExit = il.DefineLabel();
                var authoritative = il.DefineLabel();
                ...
                if (fastMethod != null)
                {
                    ...
                    il.Emit(OpCodes.Brfalse, authoritative);
                    context.EmitAddCycles(4);
                    il.Emit(OpCodes.Br, complete);
                    il.MarkLabel(authoritative);
                }

                context.EmitStoreFpuRegisterState();
                context.EmitBeginCoreInstructionCycleFloor();
                il.Emit(OpCodes.Ldarg_0);
                il.Emit(OpCodes.Ldc_I4, instruction.QuickValue);
                il.Emit(OpCodes.Ldc_I4, instruction.Register);
                ...
                il.Emit(OpCodes.Call, M68kOperationEmitter.GetM68040FpuRegisterMethod(instruction.Condition));
                il.Emit(OpCodes.Brfalse, exceptionalExit);
                context.EmitBranchToFpuRegisterExceptionalExit(exceptionalExit);
                context.EmitReloadFpuRegisterState();
                il.Emit(OpCodes.Br, complete);

                il.MarkLabel(exceptionalExit);
                context.EmitReloadState();
                il.Emit(OpCodes.Br, exit);
        ...
        private static void EmitV2M68040Fbcc(
            ILGenerator il,
            V2EmitContext context,
            M68kDecodedInstruction instruction,
            Label[] labels,
            ...
            bool recordOutOfBlockSideExit)
        {
            var evaluation = il.DeclareLocal(typeof(int));
            var notExceptional = il.DefineLabel();
            var notTaken = il.DefineLabel();
            context.EmitEvaluateFpuCondition(instruction.Condition);
            il.Emit(OpCodes.Stloc, evaluation);
            il.Emit(OpCodes.Ldloc, evaluation);
            il.Emit(OpCodes.Ldc_I4_2);
            il.Emit(OpCodes.And);
            il.Emit(OpCodes.Brfalse, notExceptional);
            EmitV2M68040ConditionalFallback(il, context, instruction, exit);

            il.MarkLabel(notExceptional);
            il.Emit(OpCodes.Ldloc, evaluation);
            il.Emit(OpCodes.Ldc_I4_1);
            il.Emit(OpCodes.And);
            ...
            EmitV2BranchToTarget(il, context, target, labels, addressToIndex, exit, root, codeStart, byteLength, false, recordOutOfBlockSideExit);
            il.MarkLabel(notTaken);
            context.EmitAddCycles(8);
        }

        private static void EmitV2M68040Fdbcc(
            ILGenerator il,
            V2EmitContext context,
            M68kDecodedInstruction instruction,
            Label[] labels,
        ...
        private static void EmitV2M68040Fscc(
            ILGenerator il,
            V2EmitContext context,
            M68kDecodedInstruction instruction,
            Label exit)
        {
            ...
            il.Emit(OpCodes.And);
            il.Emit(OpCodes.Stloc, value);
            context.EmitStoreDataRegister(instruction.Register, value, M68kOperandSize.Byte);
            context.EmitAddCycles(4);
        }

        private static void EmitV2M68040Ftrapcc(
            ILGenerator il,
            V2EmitContext context,
            M68kDecodedInstruction instruction,
            Label exit)
        ...
        private static void EmitV2M68040ConditionalFallback(
            ILGenerator il,
            V2EmitContext context,
            M68kDecodedInstruction instruction,
            Label exit)
        {
            ...
            il.Emit(OpCodes.Ldarg_0);
            EmitLoadUIntConstant(il, instruction.ProgramCounter);
            il.Emit(OpCodes.Call, ExecuteM68040FallbackForV2BatchMethod);
            il.Emit(OpCodes.Pop);
            context.EmitReloadState();
            il.Emit(OpCodes.Br, exit);
        }
            ...
            uint branchBase)
        {
            ...
            switch (operation)
            {
                ...
                case M68kJitOperation.Tst:
                    ...
                    AddCycles(GetUnaryCyclesForTiming(M68kJitOperation.Tst, source, size));
                    return true;
                case M68kJitOperation.Clr:
                    WriteEaValue(destination, size, 0);
                    SetLogicFlags(0, size);
                    AddCycles(GetUnaryCyclesForTiming(M68kJitOperation.Clr, destination, size));
                    return true;
                case M68kJitOperation.Neg:
                ...
                case M68kJitOperation.Cmp:
                    ...
                    AddCycles(GetCompareCyclesForTiming(source, size));
                    return true;
                case M68kJitOperation.Cmpi:
                    _ = Subtract(ReadEaValue(destination, size), source.Immediate, size, setExtend: false);
                    AddCycles(GetCmpiCyclesForTiming(destination, size));
                    return true;
                case M68kJitOperation.Cmpa:
            ...
            ushort command)
        {
            ...
            var fpu = BeginCompiledM68040FpuRegister(
                sourceRegister,
                destinationRegister,
                out var source,
            ...
            var execution = M68040FpuHelpers.ApplyRegisterOperation(
                ...
                opmode,
                source);
            return CompleteCompiledM68040FpuRegister(
                fpu,
                command,
                source,
                previousDestination,
        ...
        private bool TryExecuteCompiledM68040FpuMoveRegisterFast(
            int sourceRegister, int destinationRegister, int opmode, uint instructionPc)
        {
            var fpu = State.M68040Fpu;
            var source = fpu.FP[sourceRegister];
            return CompleteCompiledM68040FpuRegisterFast(
                fpu,
                instructionPc,
                M68040FpuHelpers.TryApplyRegisterMoveCanonical(fpu, destinationRegister, opmode, source));
        }

        private bool TryExecuteCompiledM68040FpuSquareRootRegisterFast(
            int sourceRegister, int destinationRegister, int opmode, uint instructionPc)
        {
            var fpu = State.M68040Fpu;
            var source = fpu.FP[sourceRegister];
            return CompleteCompiledM68040FpuRegisterFast(
                fpu,
                instructionPc,
                M68040FpuHelpers.TryApplyRegisterSquareRootCanonical(fpu, destinationRegister, opmode, source));
        }

        private bool TryExecuteCompiledM68040FpuAbsoluteRegisterFast(
            int sourceRegister, int destinationRegister, int opmode, uint instructionPc)
        {
            var fpu = State.M68040Fpu;
            var source = fpu.FP[sourceRegister];
            return CompleteCompiledM68040FpuRegisterFast(
                fpu,
                instructionPc,
                M68040FpuHelpers.TryApplyRegisterAbsoluteCanonical(fpu, destinationRegister, opmode, source));
        }

        private bool TryExecuteCompiledM68040FpuNegateRegisterFast(
            int sourceRegister, int destinationRegister, int opmode, uint instructionPc)
        {
            var fpu = State.M68040Fpu;
            var source = fpu.FP[sourceRegister];
            return CompleteCompiledM68040FpuRegisterFast(
                fpu,
                instructionPc,
                M68040FpuHelpers.TryApplyRegisterNegateCanonical(fpu, destinationRegister, opmode, source));
        }

        private bool TryExecuteCompiledM68040FpuDivideRegisterFast(
            int sourceRegister, int destinationRegister, int opmode, uint instructionPc)
        {
            var fpu = State.M68040Fpu;
            var source = fpu.FP[sourceRegister];
            var destination = sourceRegister == destinationRegister ? source : fpu.FP[destinationRegister];
            return CompleteCompiledM68040FpuRegisterFast(
                fpu,
                instructionPc,
                M68040FpuHelpers.TryApplyRegisterDivideCanonical(fpu, destinationRegister, opmode, source, destination));
        }

        private bool TryExecuteCompiledM68040FpuAddRegisterFast(
            int sourceRegister, int destinationRegister, int opmode, uint instructionPc)
        {
            var fpu = State.M68040Fpu;
            var source = fpu.FP[sourceRegister];
            ...
            return CompleteCompiledM68040FpuRegisterFast(
                fpu,
                instructionPc,
                M68040FpuHelpers.TryApplyRegisterAddCanonical(fpu, destinationRegister, opmode, source, destination));
        }

        private bool TryExecuteCompiledM68040FpuMultiplyRegisterFast(
            int sourceRegister, int destinationRegister, int opmode, uint instructionPc)
        {
            var fpu = State.M68040Fpu;
            var source = fpu.FP[sourceRegister];
            ...
            return CompleteCompiledM68040FpuRegisterFast(
                fpu,
                instructionPc,
                M68040FpuHelpers.TryApplyRegisterMultiplyCanonical(fpu, destinationRegister, opmode, source, destination));
        }

        private bool TryExecuteCompiledM68040FpuSubtractRegisterFast(
            int sourceRegister, int destinationRegister, int opmode, uint instructionPc)
        {
            var fpu = State.M68040Fpu;
            var source = fpu.FP[sourceRegister];
            ...
            return CompleteCompiledM68040FpuRegisterFast(
                fpu,
                instructionPc,
                M68040FpuHelpers.TryApplyRegisterSubtractCanonical(fpu, destinationRegister, opmode, source, destination));
        }

        private bool TryExecuteCompiledM68040FpuCompareRegisterFast(
            int sourceRegister, int destinationRegister, int opmode, uint instructionPc)
        {
            _ = opmode;
            var fpu = State.M68040Fpu;
            var source = fpu.FP[sourceRegister];
            var destination = sourceRegister == destinationRegister
                ? source
                : fpu.FP[destinationRegister];
            return CompleteCompiledM68040FpuRegisterFast(
                fpu,
                instructionPc,
                M68040FpuHelpers.TryApplyRegisterCompareCanonical(fpu, source, destination));
        }

        private bool TryExecuteCompiledM68040FpuTestRegisterFast(
            int sourceRegister, int destinationRegister, int opmode, uint instructionPc)
        {
            _ = destinationRegister;
            _ = opmode;
            var fpu = State.M68040Fpu;
            var source = fpu.FP[sourceRegister];
            return CompleteCompiledM68040FpuRegisterFast(
                fpu,
                instructionPc,
                M68040FpuHelpers.TryApplyRegisterTestCanonical(fpu, source));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private bool CompleteCompiledM68040FpuRegisterFast(
            M68040FpuState fpu,
            uint instructionPc,
            bool success)
        {
            ...
            fpu.Fpiar = instructionPc;
            _counters.NativeM68040FpuIlInstructions++;
            return true;
        }

        private bool ExecuteCompiledM68040FpuMoveRegister(
            int sourceRegister,
            int destinationRegister,
            int opmode,
            ushort command)
        {
            var fpu = BeginCompiledM68040FpuRegister(sourceRegister, destinationRegister, out var source, out var previous);
            var execution = M68040FpuHelpers.ApplyRegisterMove(fpu, destinationRegister, opmode, source, previous);
            return CompleteCompiledM68040FpuRegister(fpu, command, source, previous, execution);
        }

        private bool ExecuteCompiledM68040FpuSquareRootRegister(
            int sourceRegister,
            int destinationRegister,
            int opmode,
            ushort command)
        {
            var fpu = BeginCompiledM68040FpuRegister(sourceRegister, destinationRegister, out var source, out var previous);
            var execution = M68040FpuHelpers.ApplyRegisterSquareRoot(fpu, destinationRegister, opmode, source, previous);
            return CompleteCompiledM68040FpuRegister(fpu, command, source, previous, execution);
        }

        private bool ExecuteCompiledM68040FpuAbsoluteRegister(
            int sourceRegister,
            int destinationRegister,
            int opmode,
            ushort command)
        {
            var fpu = BeginCompiledM68040FpuRegister(sourceRegister, destinationRegister, out var source, out var previous);
            var execution = M68040FpuHelpers.ApplyRegisterAbsolute(fpu, destinationRegister, opmode, source, previous);
            return CompleteCompiledM68040FpuRegister(fpu, command, source, previous, execution);
        }

        private bool ExecuteCompiledM68040FpuNegateRegister(
            int sourceRegister,
            int destinationRegister,
            int opmode,
            ushort command)
        {
            var fpu = BeginCompiledM68040FpuRegister(sourceRegister, destinationRegister, out var source, out var previous);
            var execution = M68040FpuHelpers.ApplyRegisterNegate(fpu, destinationRegister, opmode, source, previous);
            return CompleteCompiledM68040FpuRegister(fpu, command, source, previous, execution);
        }

        private bool ExecuteCompiledM68040FpuDivideRegister(
            int sourceRegister,
            int destinationRegister,
            int opmode,
            ushort command)
        {
            var fpu = BeginCompiledM68040FpuRegister(sourceRegister, destinationRegister, out var source, out var previous);
            var execution = M68040FpuHelpers.ApplyRegisterDivide(fpu, destinationRegister, opmode, source, previous);
            return CompleteCompiledM68040FpuRegister(fpu, command, source, previous, execution);
        }

        private bool ExecuteCompiledM68040FpuAddRegister(
            int sourceRegister,
            int destinationRegister,
            int opmode,
            ushort command)
        {
            var fpu = BeginCompiledM68040FpuRegister(sourceRegister, destinationRegister, out var source, out var previous);
            var execution = M68040FpuHelpers.ApplyRegisterAdd(fpu, destinationRegister, opmode, source, previous);
            return CompleteCompiledM68040FpuRegister(fpu, command, source, previous, execution);
        }

        private bool ExecuteCompiledM68040FpuMultiplyRegister(
            int sourceRegister,
            int destinationRegister,
            int opmode,
            ushort command)
        {
            var fpu = BeginCompiledM68040FpuRegister(sourceRegister, destinationRegister, out var source, out var previous);
            var execution = M68040FpuHelpers.ApplyRegisterMultiply(fpu, destinationRegister, opmode, source, previous);
            return CompleteCompiledM68040FpuRegister(fpu, command, source, previous, execution);
        }

        private bool ExecuteCompiledM68040FpuSubtractRegister(
            int sourceRegister,
            int destinationRegister,
            int opmode,
            ushort command)
        {
            var fpu = BeginCompiledM68040FpuRegister(sourceRegister, destinationRegister, out var source, out var previous);
            var execution = M68040FpuHelpers.ApplyRegisterSubtract(fpu, destinationRegister, opmode, source, previous);
            return CompleteCompiledM68040FpuRegister(fpu, command, source, previous, execution);
        }

        private bool ExecuteCompiledM68040FpuCompareRegister(
            int sourceRegister,
            int destinationRegister,
            int opmode,
            ushort command)
        {
            var fpu = BeginCompiledM68040FpuRegister(sourceRegister, destinationRegister, out var source, out var previous);
            var execution = M68040FpuHelpers.ApplyRegisterCompare(fpu, destinationRegister, opmode, source, previous);
            return CompleteCompiledM68040FpuRegister(fpu, command, source, previous, execution);
        }

        private bool ExecuteCompiledM68040FpuTestRegister(
            int sourceRegister,
            int destinationRegister,
            int opmode,
            ushort command)
        {
            var fpu = BeginCompiledM68040FpuRegister(sourceRegister, destinationRegister, out var source, out var previous);
            var execution = M68040FpuHelpers.ApplyRegisterTest(fpu, destinationRegister, opmode, source, previous);
            return CompleteCompiledM68040FpuRegister(fpu, command, source, previous, execution);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private M68040FpuState BeginCompiledM68040FpuRegister(
            int sourceRegister,
            int destinationRegister,
            out ExtF80 source,
            out ExtF80 previousDestination)
        {
            _counters.NativeM68040FpuIlInstructions++;
            var fpu = State.M68040Fpu;
            fpu.Fpiar = State.LastInstructionProgramCounter;
            source = fpu.FP[sourceRegister];
            previousDestination = fpu.FP[destinationRegister];
            return fpu;
        }

        private bool CompleteCompiledM68040FpuRegister(
            M68040FpuState fpu,
            ushort command,
            ExtF80 source,
            ExtF80 previousDestination,
            M68040FpuExecutionResult execution)
        {
            if (!execution.Supported)
            {
                _compiledM68040FpuExceptionPending = true;
        ...
        private static int GetM68040FpuConditionTruthMask(int condition)
        {
            var mask = 0;
            for (var status = 0; status < 16; status++)
            {
                var nan = (status & 0x1) != 0;
                var zero = (status & 0x4) != 0;
                var negative = (status & 0x8) != 0;
                var matches = (condition & 0x0F) switch
                {
                    0x0 => false,
                    0x1 => zero,
            ...
            ushort sourceExtension1)
        {
            ...
            AddCycles(GetPeaCyclesForTiming(source));
        }
        ...
        private void ExecuteNeg(M68kDecodedEa destination, M68kOperandSize size)
        {
            ...
            AddCycles(GetUnaryCyclesForTiming(M68kJitOperation.Neg, destination, size));
        }
        ...
        private void ExecuteNegx(M68kDecodedEa destination, M68kOperandSize size)
        {
            ...
            AddCycles(GetUnaryCyclesForTiming(M68kJitOperation.Negx, destination, size));
        }
        ...
        private void ExecuteNot(M68kDecodedEa destination, M68kOperandSize size)
        {
            ...
            AddCycles(GetUnaryCyclesForTiming(M68kJitOperation.Not, destination, size));
        }
        ...
        private void ExecuteNbcd(M68kDecodedEa destination)
        {
            ...
            SetBcdFlags(result, carry, overflow);
            AddCycles(memory ? 8 + GetEaOperandCyclesForTiming(destination, M68kOperandSize.Byte) : 6);
        }
            ...
            bool registerToEa)
        {
            ...
            AddCycles(registerToEa
                ? GetAluDataToEaCyclesForTiming(destination, size)
                : GetAluEaToDataCyclesForTiming(source, size));
            return true;
        }
            ...
            M68kOperandSize size)
        {
            ...
            AddCycles(GetImmediateAluCyclesForTiming(destination, size));
            return true;
        }
        ...
        private uint ReadPhysicalMemoryValue(uint address, M68kOperandSize size)
        {
            return size switch
            {
        ...
        private void WritePhysicalMemoryValue(uint address, uint value, M68kOperandSize size)
        {
            if (size == M68kOperandSize.Byte)
            {
        ...
        internal static int EstimateMoveCycles(M68kDecodedEa source, M68kDecodedEa destination, M68kOperandSize size)
        {
            ...
            {
                return 4 + GetEaOperandCyclesForTiming(source, size);
            }
        ...
        private static int GetMoveDestinationEaCyclesForTiming(M68kDecodedEa destination)
        {
            return destination.Kind switch
            {
                M68kJitEaKind.AddressIndirect or
                M68kJitEaKind.AddressPostincrement or
                M68kJitEaKind.AddressPredecrement => 4,
                M68kJitEaKind.AddressDisplacement or
                M68kJitEaKind.AbsoluteWord => 8,
        ...
        private static int GetPeaCyclesForTiming(M68kDecodedEa source)
        {
            return source.Kind switch
            {
                M68kJitEaKind.AddressIndirect => 12,
                M68kJitEaKind.AddressDisplacement or
                M68kJitEaKind.AbsoluteWord or
                M68kJitEaKind.PcDisplacement => 16,
                M68kJitEaKind.AddressIndex or
                M68kJitEaKind.AbsoluteLong or
                M68kJitEaKind.PcIndex => 20,
                _ => 12
            };
        }
        ...
        internal static int GetUnaryCyclesForTiming(
            ...
            M68kOperandSize size)
        {
            if (ea.Kind == M68kJitEaKind.DataRegister)
            {
                return operation == M68kJitOperation.Tst || size != M68kOperandSize.Long ? 4 : 6;
            }

            var baseCycles = operation == M68kJitOperation.Tst
                ? 4
                : size == M68kOperandSize.Long ? 12 : 8;
            return baseCycles + GetEaOperandCyclesForTiming(ea, size);
        }

        internal static int GetCompareCyclesForTiming(M68kDecodedEa source, M68kOperandSize size)
        {
        ...
        internal static int GetCmpiCyclesForTiming(M68kDecodedEa destination, M68kOperandSize size)
        {
            ...
            return (size == M68kOperandSize.Long ? 12 : 8) + GetEaOperandCyclesForTiming(destination, size);
        }

        internal static int GetAluEaToDataCyclesForTiming(M68kDecodedEa source, M68kOperandSize size)
        {
            if (source.Kind is M68kJitEaKind.DataRegister or M68kJitEaKind.AddressRegister)
            {
        ...
        internal static int GetAluDataToEaCyclesForTiming(M68kDecodedEa destination, M68kOperandSize size)
        {
            if (destination.Kind == M68kJitEaKind.DataRegister)
            {
                return size == M68kOperandSize.Long ? 8 : 4;
        ...
        internal static int GetImmediateAluCyclesForTiming(M68kDecodedEa destination, M68kOperandSize size)
        {
            if (destination.Kind == M68kJitEaKind.DataRegister)
            {
        ...
        private sealed class V2EmitContext
        {
            ...
            private static readonly PropertyInfo M68040FpuProperty =
                typeof(M68kCpuState).GetProperty(
                    nameof(M68kCpuState.M68040Fpu),
                    BindingFlags.Instance | BindingFlags.NonPublic) ??
                throw new MissingMemberException(typeof(M68kCpuState).FullName, nameof(M68kCpuState.M68040Fpu));
            private static readonly PropertyInfo M68040FpsrProperty =
                typeof(M68040FpuState).GetProperty(nameof(M68040FpuState.Fpsr)) ??
                throw new MissingMemberException(typeof(M68040FpuState).FullName, nameof(M68040FpuState.Fpsr));
            ...
            private static readonly FieldInfo CompiledM68040FpuExceptionPendingField =
                typeof(M68kJitCore).GetField(nameof(_compiledM68040FpuExceptionPending), BindingFlags.Instance | BindingFlags.NonPublic) ??
                throw new MissingFieldException(typeof(M68kJitCore).FullName, nameof(_compiledM68040FpuExceptionPending));
            ...
            {
                FpuExceptionDirtyDataRegisters = dirtyDataRegisters;
                FpuExceptionDirtyAddressRegisters = dirtyAddressRegisters;
            }
                ...
                int loadAddressRegisters = 0xFF)
            {
                ...
                _il.Emit(OpCodes.Stfld, CompiledM68040FpuExceptionPendingField);
                _il.Emit(OpCodes.Ldarg_0);
            ...
            public void EmitStoreFpuRegisterState()
                => EmitStoreState(
                    recordLazyWriteback: false,
                    FpuExceptionDirtyDataRegisters,
                    FpuExceptionDirtyAddressRegisters);
            ...
            public void EmitReloadFpuRegisterState()
            {
                _il.Emit(OpCodes.Ldloc, State);
                _il.Emit(OpCodes.Call, ProgramCounterProperty.GetMethod!);
                _il.Emit(OpCodes.Stloc, ProgramCounter);
            ...
            public void EmitEvaluateFpuCondition(int condition)
            {
                var fpsr = _il.DeclareLocal(typeof(uint));
                var normal = _il.DefineLabel();
                var done = _il.DefineLabel();

                _il.Emit(OpCodes.Ldloc, State);
                _il.Emit(OpCodes.Call, M68040FpuProperty.GetMethod!);
                _il.Emit(OpCodes.Call, M68040FpsrProperty.GetMethod!);
                _il.Emit(OpCodes.Stloc, fpsr);

                if ((condition & 0x10) != 0)
                {
                    _il.Emit(OpCodes.Ldloc, fpsr);
                    EmitLoadUIntConstant(_il, M68040FpuState.ConditionNan);
                    _il.Emit(OpCodes.And);
                    _il.Emit(OpCodes.Brfalse, normal);
                    _il.Emit(OpCodes.Ldc_I4_2);
                    _il.Emit(OpCodes.Br, done);
                    _il.MarkLabel(normal);
                }

                _il.Emit(OpCodes.Ldc_I4, GetM68040FpuConditionTruthMask(condition));
                _il.Emit(OpCodes.Ldc_I4_1);
                _il.Emit(OpCodes.Ldloc, fpsr);
                _il.Emit(OpCodes.Ldc_I4, 24);
                _il.Emit(OpCodes.Shr_Un);
                _il.Emit(OpCodes.Ldc_I4, 0x0F);
                _il.Emit(OpCodes.And);
            ...
            public void EmitBranchToFpuRegisterExceptionalExit(Label exceptionalExit)
            {
                _il.Emit(OpCodes.Ldarg_0);
                _il.Emit(OpCodes.Ldfld, CompiledM68040FpuExceptionPendingField);
                _il.Emit(OpCodes.Brtrue, exceptionalExit);
                _il.Emit(OpCodes.Ldloc, State);
                _il.Emit(OpCodes.Call, ProgramCounterProperty.GetMethod!);
                _il.Emit(OpCodes.Ldloc, ProgramCounter);
                _il.Emit(OpCodes.Bne_Un, exceptionalExit);
            }
    ...
    internal struct M68kJitCounters
    {
        ...
        public long M68040NativeFpuConditionSites { get; set; }

        public long M68040FpuBsunSlowPathExits { get; set; }

        public long M68040FpuTrapSlowPathExits { get; set; }
