...
namespace Copper68k
{
    ...
    public interface IM68kBus
    {
        ...
        /// <summary>
        /// Invokes a host gateway and describes how execution continues after the
        /// host routine has completed.  The legacy boolean entry point remains a
        /// completed-gateway compatibility path.
        /// </summary>
        M68kHostGatewayInvocation InvokeHostGateway(uint instructionProgramCounter, uint token, M68kCpuState state)
            => TryInvokeHostGateway(instructionProgramCounter, token, state)
                ? new(true, M68kHostGatewayResult.Completed)
                : default;
    ...
    /// <summary>Post-call control flow selected by an atomic host gateway.</summary>
    public enum M68kHostGatewayResult
    {
        /// <summary>Complete the gateway and perform its normal emulated return.</summary>
        Completed,
        /// <summary>Leave the call frame intact and yield at the next outer instruction boundary.</summary>
        BlockCurrentTask,
        /// <summary>Complete normally and request a task selection at the next outer boundary.</summary>
        Reschedule
    }

    /// <summary>Result of looking up and invoking a host gateway.</summary>
    public readonly record struct M68kHostGatewayInvocation(bool Handled, M68kHostGatewayResult Result);
    ...
    internal readonly record struct M68kDeferredCpuBusCheckpointRequest(
        ...
        long PendingSuccessorVirtualCycle = 0,
        ulong PendingSuccessorPredecessorToken = 0,
        bool PendingSuccessorRequiresIrcGap = false)
    {
        public ulong LastInterruptSampleToken { get; init; }
        public ulong InstructionInterruptSampleToken { get; init; }
    }

    internal readonly record struct M68kDeferredCpuFetchResolution(
        ulong Token,
        long ReadyCycle);

    internal readonly record struct M68kDeferredCpuBusCheckpoint(
        long ArchitecturalRetireCycle,
        long PhysicalNextTransferCycle,
        long LastReadyCycle,
        long QueueReadyCycle0,
        long QueueReadyCycle1,
        byte ResolvedQueueMask,
        bool HasTrimmedPendingTransition = false,
        long TrimmedPendingOriginalCycle = 0,
        long ResolvedPendingRequestCycle = 0,
        ulong TrimmedPendingPredecessorToken = 0,
        bool TrimmedPendingRequiresIrcGap = false,
        bool PendingSuccessorBlocksInstructionEntry = false,
        bool RebaseCpuTimeline = false)
    {
        // A bus replay cursor is not necessarily the CPU's next transfer
        // phase. Null means that this checkpoint committed no known transfer;
        // cycle zero is a valid resolved CPU phase.
        public long? ResolvedCpuNextBusCycle { get; init; }

        // Echo the resolved identity as well as its phase: an empty-journal
        // checkpoint can reuse an earlier result after the queue has changed.
        public M68kDeferredCpuFetchResolution LastInterruptSampleResolution { get; init; }
        public M68kDeferredCpuFetchResolution InstructionInterruptSampleResolution { get; init; }
    }

    internal readonly record struct M68000InterruptSample(
        long Cycle,
        ulong DeferredFetchToken = 0)
    {
        // Zero denotes an already-physical sample. Equal cycle numbers do not
        // associate that sample with an unrelated deferred transfer.
        public M68000InterruptSample ResolveDeferredFetch(
            M68kDeferredCpuFetchResolution resolution)
        {
            if (DeferredFetchToken == 0 || DeferredFetchToken != resolution.Token)
            {
                return this;
            }

            return resolution.ReadyCycle == long.MinValue
                ? this
                : new M68000InterruptSample(resolution.ReadyCycle);
        }

        public M68000InterruptSample ResolveDeferredFetch(ulong token, long readyCycle)
            => ResolveDeferredFetch(new M68kDeferredCpuFetchResolution(token, readyCycle));
    }

    internal readonly record struct M68kDeferredCpuTrimmedInstructionFetch(
        ulong FirstToken,
        M68kInstructionFetchPublicationPhase PublicationPhase,
        long RetirementFloor,
        M68kInstructionFetchPublicationContext PublicationContext,
        long VirtualRequestedCycle);
    ...
    internal interface IM68kDeferredCpuInstructionTiming
    {
        ...
        bool IsInternalNoBusWindowEnabled { get; }

        bool HasPhysicalCpuBusCursor => true;
        ...
        void RegisterDeferredCpuInterruptSamples(ulong lastSampleToken, ulong instructionSampleToken)
        {
        }

        M68kDeferredCpuFetchResolution GetDeferredCpuInterruptSampleResolution(ulong token)
            => default;
        ...
        bool TryAdvanceInternalNoBusWindow(
            M68kCpuState state,
            long currentCycle,
            int cycles,
    ...
    internal enum M68kInternalNoBusWindowKind
    {
        Unknown = 0,
        Multiply = 1,
        Divide = 2
    ...
    internal interface IM68000BusCycleTiming
    {
        int M68000BusCycleStartDelay { get; }

        bool RequiresExactM68000PipelineFallback { get; }

        M68000BusAccessTiming GetM68000BusAccessTiming(
            uint address,
            M68kOperandSize size,
            M68kBusAccessKind accessKind,
            bool isWrite,
    ...
    internal enum M68kInstructionFetchPublicationPhase : byte
    {
        Required = 0,
        RetirementQueue = 1,
    ...
    internal readonly record struct M68kInstructionFetchPublicationContext(
        ...
        long EntryBusCycle,
        int EntryPrefetchCount,
        long EntryReadyCycle0,
        long EntryReadyCycle1,
        ulong EntryTimingToken0,
        ulong EntryTimingToken1,
    ...
    internal interface IM68kInstructionFetchWindowBus
    {
        ...
        void CommitInstructionFetchWindowWord(
        ...
        ushort ReadInstructionFetchWindowWord(
            in M68kInstructionFetchWindow window,
            uint address,
            ref long cycle,
            M68kInstructionFetchPublicationPhase publicationPhase = M68kInstructionFetchPublicationPhase.Required,
    ...
    public interface IM68kCore : IDisposable
    {
        ...
        /// <summary>
        /// Replaces task-local architectural state while retaining global machine
        /// time and clearing backend execution state owned by the previous task.
        /// </summary>
        void SwitchTaskContext(M68kCpuState next) => State.CopyTaskContextFrom(next);
    ...
    {
        public bool PendingPrefetchAddressPhaseIssued { get; init; }
    }
    ...
    internal readonly record struct M68000PipelineState(
        uint PrefetchAddress,
        ushort Word0,
        ushort Word1,
        long ReadyCycle0,
        long ReadyCycle1,
        bool DeferredBatchEligible0,
        bool DeferredBatchEligible1,
        int PrefetchCount,
        bool ConsumeWithoutPrefetch,
        bool SkipRetirePrefetchTopUp,
        ...
        long LastInterruptSampleCycle = long.MinValue)
    {
        public ulong LastBusReadyTimingToken { get; init; }
        public ulong LastInterruptSampleTimingToken { get; init; }
        public bool PendingPrefetchAddressPhaseIssued { get; init; }
    }

    internal interface IM68000PipelineStateTransfer
    {
        M68000PipelineState ExportM68000PipelineState();

        void ImportM68000PipelineState(in M68000PipelineState state);
    }
    ...
    public sealed class M68kCpuState
    {
        ...
        /// <summary>
        /// Replaces only state which belongs to an emulated task.  Machine time
        /// and CPU run-state are global and must never be restored by Exec task
        /// switching.
        /// </summary>
        public void CopyTaskContextFrom(M68kCpuState source)
        {
            ArgumentNullException.ThrowIfNull(source);
            Array.Copy(source.D, D, D.Length);
            Array.Copy(source.A, A, A.Length);
        ...
        /// <summary>
        /// Updates the negative, zero, overflow, and carry condition codes for a
        /// logical result of any operand size in a single status-register update.
        /// </summary>
        /// <param name="value">The result to test.</param>
        /// <param name="size">The operand size that defines the mask and sign bit.</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void SetSizedLogicFlags(uint value, M68kOperandSize size)
        {
            value &= Mask(size);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void SetLongLogicFlags(uint value)
        {
            var status = _statusRegister & unchecked((ushort)~(Negative | Zero | Overflow | Carry));
            if (value == 0)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void SetLongArithmeticFlags(uint value, bool overflow, bool carry)
        {
            var status = _statusRegister & unchecked((ushort)~(Negative | Zero | Overflow | Carry | Extend));
            if (value == 0)
            {
                status |= Zero;
            }
        ...
        where TCpuDataAccess : struct, IM68kCpuDataAccess<TBus, TCpuDataAccess>
    {
        ...
        private sealed class M68000PrefetchQueue
        {
            ...
            public ulong TimingToken0;
            public ulong TimingToken1;
            ...
            public M68kInstructionFetchPublicationPhase PendingPublicationPhase;
            public long PendingRetirementFloor = long.MinValue;
            public M68kInstructionFetchPublicationContext PendingPublicationContext;
            public long PendingVirtualRequestedCycle;
            public bool PendingBlocksInstructionEntry;
            public bool PendingAddressPhaseIssued;
        ...
        private struct M68000FixedBatchContext
        {
            ...
            private long _readyBusCycle;
            public ulong ReadyBusTimingToken;
            public long ReadyBusCycle
            {
                readonly get => _readyBusCycle;
                set
                {
                    _readyBusCycle = value;
                    ReadyBusTimingToken = 0;
                }
            }
            public long RetireBusCycle;
            public long ExceptionEntryNotBeforeCycle;
            ...
            public ulong PrefetchTimingToken0;
            public ulong PrefetchTimingToken1;
            ...
            public bool PendingPrefetchBlocksInstructionEntry;
            public bool PendingPrefetchAddressPhaseIssued;
            public bool ConsumeWithoutPrefetch;
            ...
            public M68000InterruptSample InstructionInterruptSample;
            public M68000InterruptSample LastInterruptSample;
            public long InstructionInterruptSampleCycle
            {
                readonly get => InstructionInterruptSample.Cycle;
                set => InstructionInterruptSample = new M68000InterruptSample(value);
            }
            public long LastInterruptSampleCycle
            {
                readonly get => LastInterruptSample.Cycle;
                set => LastInterruptSample = new M68000InterruptSample(value);
            }
        ...
        {
            public bool Valid;
            public bool ContainsFastMemory;
            public bool HasConditionalFlow;
            public bool SinglePathLoop;
            ...
            public byte SinglePathLoopStartIndex;
        }
        ...
        private struct M68000FixedPlanRunInstruction
        {
            ...
            public byte FallthroughIndex;
            public byte TakenIndex;
        }
        ...
        private readonly bool _useM68020BriefIndexedAddressing;
        ...
        private ulong _instructionEntryTimingToken0;
        private ulong _instructionEntryTimingToken1;
        private M68000InterruptSample _instructionInterruptSample;
        private M68000InterruptSample _lastInterruptSample;
        private ulong _cpuBusReadyTimingToken;
        private bool _hasRegisteredDeferredInterruptSamples;
        private long _instructionInterruptSampleCycle
        {
            get => _instructionInterruptSample.Cycle;
            set => _instructionInterruptSample = new M68000InterruptSample(value);
        }
        private long _lastInterruptSampleCycle
        {
            get => _lastInterruptSample.Cycle;
            set => _lastInterruptSample = new M68000InterruptSample(value);
        }
        ...
        private ulong _instructionDeferredCpuTimingDependencyToken;
        private long _instructionDeferredCpuTimingDependencyVirtualReadyCycle = -1;
        private long _instructionFetchPublicationGroup;
        ...
        private ulong _prefetchTimingToken0 { get => _prefetchQueue.TimingToken0; set => _prefetchQueue.TimingToken0 = value; }
        private ulong _prefetchTimingToken1 { get => _prefetchQueue.TimingToken1; set => _prefetchQueue.TimingToken1 = value; }
        ...
        private bool _pendingPrefetchAddressPhaseIssued
        {
            get => _prefetchQueue.PendingAddressPhaseIssued;
            set => _prefetchQueue.PendingAddressPhaseIssued = value;
        }
        private M68kInstructionFetchPublicationPhase _pendingPrefetchPublicationPhase
        {
            get => _prefetchQueue.PendingPublicationPhase;
            set => _prefetchQueue.PendingPublicationPhase = value;
        }
        private long _pendingPrefetchRetirementFloor
        {
            get => _prefetchQueue.PendingRetirementFloor;
            set => _prefetchQueue.PendingRetirementFloor = value;
        }
        private M68kInstructionFetchPublicationContext _pendingPrefetchPublicationContext
        {
            get => _prefetchQueue.PendingPublicationContext;
            set => _prefetchQueue.PendingPublicationContext = value;
        }
        private long _pendingPrefetchVirtualRequestedCycle
        {
            get => _prefetchQueue.PendingVirtualRequestedCycle;
            set => _prefetchQueue.PendingVirtualRequestedCycle = value;
        }
        private bool _pendingPrefetchBlocksInstructionEntry
        {
            get => _prefetchQueue.PendingBlocksInstructionEntry;
            set => _prefetchQueue.PendingBlocksInstructionEntry = value;
        }
        ...
        {
            get => _busTimeline.LastReadyCycle;
            set
            {
                _busTimeline.LastReadyCycle = value;
                _cpuBusReadyTimingToken = 0;
            }
        }
        ...
        internal M68kInterpreterCore(
            ...
            bool enableOpcodePlan = true,
            M68kOpcodePlanDispatch opcodePlanDispatch = M68kOpcodePlanDispatch.KindTable,
            bool useM68020BriefIndexedAddressing = false)
        {
            ...
            _useM68020BriefIndexedAddressing = useM68020BriefIndexedAddressing;
        ...
        internal int ExecuteInstructions(int maxInstructions, long? targetCycle, IM68kInstructionBoundary boundary)
        {
            ...
            var fastMemoryRunAdmissionEnabled =
                ...
                !_instructionFrequency.Enabled &&
                busAccessBatchBoundary != null;
            var cachedRunAdmissionRetryInstructions = 0;
            while (!State.Halted &&
                ...
                (!targetCycle.HasValue || State.Cycles < targetCycle.Value))
            {
                ...
                if (CanAttemptDeferredCpuBusBatch(maxInstructions - instructions) &&
                ...
                if (cachedRunAdmissionRetryInstructions != 0)
                {
                    cachedRunAdmissionRetryInstructions--;
                }
                else if ((fixedPlanRunAdmissionEnabled || fastMemoryRunAdmissionEnabled) &&
                    _prefetchCount > 0 &&
                    _prefetchAddress == State.ProgramCounter)
                {
                    if (!_hasPendingPrefetch &&
                        IsFixedPlanRunEntryOpcode(
                            _prefetchWord0,
                            allowFastMemory: _fastMemoryBus != null) &&
                        TryExecuteCachedFixedPlanGraph(
                            maxInstructions - instructions,
                            targetCycle,
                            boundary,
                    ...
                    // every unsupported instruction. A fast-memory bus keeps
                    // its admission alive across the whole call, so back the
                    // probe off for a bounded instruction window instead of
                    // rescanning the window on every scalar instruction.
                    if (!_hasPendingPrefetch)
                    {
                        fixedPlanRunAdmissionEnabled = false;
                        cachedRunAdmissionRetryInstructions =
                            CachedFixedPlanRunAdmissionRetryInstructionCount;
                    }
        ...
        public M68000PrefetchDiagnosticState CapturePrefetchDiagnosticState()
            ...
            {
                PendingPrefetchAddressPhaseIssued =
                    _pendingPrefetchAddressPhaseIssued
            };
        ...
        private int ExecuteSingleInstruction()
        {
            ...
            if (_deferredCpuBusBatchExecutionActive &&
                _prefetchCount == 0 &&
                _hasPendingPrefetch &&
            ...
            try
            {
                    ...
                    !EnsureDeferredCpuBusBatchPrefetchReady())
                {
                    throw new InvalidOperationException(
                        "Deferred CPU batch prefetch eligibility changed after instruction entry.");
                }
        ...
        // A failed cached fixed-plan-run probe rescans the whole run window.
        // Amortize the next probe across enough scalar instructions to cover
        // that scan instead of paying it on every unsupported instruction.
        private const int CachedFixedPlanRunAdmissionRetryInstructionCount = 24;
            ...
            out int executedInstructions)
        {
            ...
            var allowChipInstructionFetchEpoch =
                !IsMemoryFormMoveOpcode(_prefetchWord0) &&
            ...
            try
            {
                ...
                {
                    if (!CanEnterDeferredCpuBusBatchInstruction())
                    {
                    ...
                    if (deferredCpuInstructionTiming.RequiresControlFlowBatchBoundary &&
                        IsDeferredCpuBatchControlFlowOpcode(State.LastOpcode))
                    {
                        reason = M68kDeferredCpuBusBatchExitReason.PcLeftFastWindow;
                        break;
                    }
                    if (targetCycle.HasValue &&
                        deferredCpuInstructionTiming.IsDeferredCpuBusBatchActive &&
                            ...
                            out _))
                    {
                        if (deferredCpuInstructionTiming.TryTrimDeferredCpuInstructionFetchSuffix(
                                _instructionDeferredCpuTimingDependencyToken,
                                out var trimmedFetch))
                        {
                            TrimDeferredPrefetchQueueSuffix(in trimmedFetch);
                        }

                        var synchronizedCycle = State.Cycles;
                        var checkpointRequest = new M68kDeferredCpuBusCheckpointRequest(
                            ...
                            _hasPendingPrefetch,
                            _pendingPrefetchVirtualRequestedCycle,
                            _pendingPrefetchPublicationContext.PredecessorToken,
                            _pendingPrefetchPublicationContext.RequiresIrcGap)
                        {
                            LastInterruptSampleToken = _lastInterruptSample.DeferredFetchToken,
                            InstructionInterruptSampleToken = _instructionInterruptSample.DeferredFetchToken
                        };
                        var checkpoint = deferredCpuInstructionTiming.SynchronizeDeferredCpuBusBatchTiming(
                            ref synchronizedCycle,
                            in checkpointRequest);
                        ApplyDeferredCpuBusCheckpoint(
                            synchronizedCycle,
                            in checkpointRequest,
                            in checkpoint,
                            finalBoundary: false);
            ...
            finally
            {
                ...
                if (deferredCpuInstructionTiming.IsDeferredCpuBusBatchActive)
                {
                    var trimmedCancellableFetchSuffix = false;
                    if ((reason is M68kDeferredCpuBusBatchExitReason.MaxInstructions or
                            M68kDeferredCpuBusBatchExitReason.TargetCycle or
                            M68kDeferredCpuBusBatchExitReason.Completed) &&
                        deferredCpuInstructionTiming.TryTrimDeferredCpuInstructionFetchSuffix(
                            _instructionDeferredCpuTimingDependencyToken,
                            out var trimmedFetch))
                    {
                        TrimDeferredPrefetchQueueSuffix(in trimmedFetch);
                        trimmedCancellableFetchSuffix = true;
                    }
                    ...
                    var mustMaterializeEmptyPrefetch =
                        !trimmedCancellableFetchSuffix &&
                        hasEmptyPendingPrefetch &&
                        reason == M68kDeferredCpuBusBatchExitReason.Completed;
                    if (!trimmedCancellableFetchSuffix &&
                        (reason == M68kDeferredCpuBusBatchExitReason.Completed ||
                    ...
                    deferredCpuInstructionTiming.EndDeferredCpuBusBatch(ref cycle, reason);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool IsDeferredCpuBatchControlFlowOpcode(ushort opcode)
            => (opcode & 0xF000) == 0x6000 ||
                (opcode & 0xF0F8) == 0x50C8 ||
                (opcode & 0xFFC0) is 0x4E80 or 0x4EC0 ||
                opcode is 0x4E73 or 0x4E74 or 0x4E75 or 0x4E77;

        private void ApplyDeferredCpuBusCheckpoint(
            long synchronizedCycle,
            in M68kDeferredCpuBusCheckpointRequest checkpointRequest,
            in M68kDeferredCpuBusCheckpoint checkpoint,
            bool finalBoundary)
        {
            if (checkpoint.RebaseCpuTimeline)
            {
                // A deferred Chip-fetch batch has committed its physical
                // transfers inside Agnus already.  Its speculative core
                // timeline can be ahead of the CPU-visible checkpoint, so
                // replace the three CPU cursors as one coherent snapshot.
                State.Cycles = synchronizedCycle;
            }
            else
            {
                State.Cycles = Math.Max(State.Cycles, synchronizedCycle);
            }

            // Samples can refer to a consumed fetch rather than either queued
            // word. Resolve by the identity echoed by replay, never by a cycle
            // coincidence or by the checkpoint's later retirement cursor.
            _lastInterruptSample = _lastInterruptSample.ResolveDeferredFetch(
                checkpoint.LastInterruptSampleResolution);
            _instructionInterruptSample = _instructionInterruptSample.ResolveDeferredFetch(
                checkpoint.InstructionInterruptSampleResolution);
            RegisterDeferredInterruptSamples();
            if ((checkpoint.ResolvedQueueMask & 1) != 0 &&
                _prefetchTimingToken0 == checkpointRequest.QueueToken0)
            {
                _prefetchCompletedCycle0 = checkpoint.QueueReadyCycle0;
                if (_prefetchCount == 1)
                {
                    _prefetchCompletedCycle1 = checkpoint.QueueReadyCycle0;
                }
            }
            if ((checkpoint.ResolvedQueueMask & 2) != 0 &&
                _prefetchTimingToken1 == checkpointRequest.QueueToken1)
            {
                _prefetchCompletedCycle1 = checkpoint.QueueReadyCycle1;
            }

            if (checkpoint.ResolvedCpuNextBusCycle is { } nextBusCycle)
            {
                // Replay supplies the same next-transfer result as a scalar
                // access. Neither the executor horizon nor the ready phase
                // identifies this cursor without that bus timing contract.
                _cpuBusCycle = checkpoint.RebaseCpuTimeline
                    ? nextBusCycle
                    : Math.Max(_cpuBusCycle, nextBusCycle);
            }
            if (checkpoint.RebaseCpuTimeline)
            {
                _cpuBusReadyCycle = checkpoint.LastReadyCycle;
                _cpuRetireBusCycle = checkpoint.ArchitecturalRetireCycle;
            }
            else
            {
                _cpuBusReadyCycle = Math.Max(
                    _cpuBusReadyCycle,
                    checkpoint.LastReadyCycle);
                _cpuRetireBusCycle = Math.Max(
                    _cpuRetireBusCycle,
                    checkpoint.ArchitecturalRetireCycle);
            }
            if (_hasPendingPrefetch &&
                checkpoint.HasTrimmedPendingTransition &&
                checkpoint.TrimmedPendingOriginalCycle ==
                    _pendingPrefetchVirtualRequestedCycle)
            {
                _pendingPrefetchEarliestCycle = checkpoint.RebaseCpuTimeline
                    ? checkpoint.ResolvedPendingRequestCycle
                    : Math.Max(
                        _pendingPrefetchEarliestCycle,
                        checkpoint.ResolvedPendingRequestCycle);
                _pendingPrefetchBlocksInstructionEntry |=
                    checkpoint.PendingSuccessorBlocksInstructionEntry;
            }
            if (!checkpoint.RebaseCpuTimeline)
            {
                RebasePendingPrefetchAfterDeferredSynchronization();
            }
        }

        private void TrimDeferredPrefetchQueueSuffix(
            in M68kDeferredCpuTrimmedInstructionFetch trimmedFetch)
        {
            var firstTrimmedToken = trimmedFetch.FirstToken;
            while (_prefetchCount > 0)
            {
                var tailToken = _prefetchCount == 2
                    ? _prefetchTimingToken1
                    : _prefetchTimingToken0;
                if (tailToken < firstTrimmedToken)
                {
                    break;
                }
            ...
            var startDelay = _m68000BusCycleTiming?.M68000BusCycleStartDelay ?? 0;
            var pendingAddress = unchecked(
                _prefetchAddress + (uint)(_prefetchCount * 2));
            var earliestCycle = State.Cycles + startDelay;
            _hasPendingPrefetch = true;
            _pendingPrefetchAddress = pendingAddress;
            _pendingPrefetchEarliestCycle = earliestCycle;
            _pendingPrefetchPublicationPhase = trimmedFetch.PublicationPhase;
            _pendingPrefetchRetirementFloor = trimmedFetch.RetirementFloor;
            _pendingPrefetchPublicationContext = trimmedFetch.PublicationContext;
            _pendingPrefetchVirtualRequestedCycle = trimmedFetch.VirtualRequestedCycle;
            _pendingPrefetchBlocksInstructionEntry = false;
            _pendingPrefetchAddressPhaseIssued = false;
            _pendingPrefetchEarliestCycle = Math.Max(
                _pendingPrefetchEarliestCycle,
                trimmedFetch.VirtualRequestedCycle);
            _cpuBusCycle = earliestCycle;
        }

        private void RebasePendingPrefetchAfterDeferredSynchronization()
        {
            if (!_hasPendingPrefetch)
            {
                return;
            ...
            _pendingPrefetchEarliestCycle = Math.Max(
                _pendingPrefetchEarliestCycle,
                Math.Max(earliestCycle, _cpuBusCycle));
            _pendingPrefetchVirtualRequestedCycle = Math.Max(
                _pendingPrefetchVirtualRequestedCycle,
                Math.Max(earliestCycle, _cpuBusCycle));
            _cpuBusCycle = Math.Max(_cpuBusCycle, _pendingPrefetchEarliestCycle);
            if (_prefetchCount == 0)
            {
                // Keep the empty queue's frozen diagnostic/transition state in
                // the same shape as scalar retirement.
                _prefetchCompletedCycle0 = State.Cycles;
                _prefetchCompletedCycle1 = State.Cycles;
            }
        }
        ...
        private bool TryExecuteCachedFixedPlanGraph(
            int maxInstructions,
            long? targetCycle,
            IM68kInstructionBoundary boundary,
            out int executedInstructions)
        {
            ...
            var entryWord1 = _prefetchCount > 1
                ? _prefetchWord1
                : runWindow.ReadWord(unchecked(State.ProgramCounter + 2));
            var allowFastMemory = _fastMemoryBus != null;
            ...
            var pureBoundary = boundary as IM68kPureCpuTraceBatchBoundary;
            var memoryBoundary = boundary as IM68kBusAccessTraceBatchBoundary;
            var previousCycle = State.Cycles;
            long batchTargetCycle;
            if (slot.ContainsFastMemory)
            {
                if (memoryBoundary == null ||
                    !memoryBoundary.TryBeginBusAccessTraceBatch(
                        State,
                        targetCycle.Value,
                        out batchTargetCycle))
                {
                    return false;
                }
            }
            else if (pureBoundary == null ||
                !pureBoundary.TryBeginPureCpuTraceBatch(
                    State,
                    targetCycle.Value,
                    out batchTargetCycle))
            {
            ...
            var instructionIndex = FindFixedPlanRunInstructionIndex(
                instructionBase,
            ...
            try
            {
                if (!slot.SinglePathLoop)
                {
                    ExecuteCachedFixedPlanGraphWalk(
                        ref context,
                        ref registers,
                        in runWindow,
                        slotIndex,
                    ...
                    instructionIndex = FixedPlanRunExitIndex;
                }
                    ...
                    instructionIndex < slot.InstructionCount)
                {
                    ...
                    if (!loopStartCaptured &&
                        instructionIndex == slot.SinglePathLoopStartIndex)
                    {
                        loopStartContext = context;
                        loopStartCaptured = true;
                    }

                    if (!TryExecuteCachedFixedPlanGraphInstruction(
                        ref context,
                        ref registers,
                        in instruction,
                        out var nextInstructionIndex,
                        out validateWindow))
                    {
                        break;
                    }
                    ...
                    executedInstructions++;
                    if (loopStartCaptured &&
                        nextInstructionIndex == slot.SinglePathLoopStartIndex &&
                        !_plannedInterpreterCountersEnabled &&
            ...
            if (slot.ContainsFastMemory)
            {
                memoryBoundary!.AfterBusAccessTraceBatch(
                    previousCycle,
                    State.Cycles,
                    executedInstructions);
            }
            ...
            {
                pureBoundary!.AfterPureCpuTraceBatch(
                    previousCycle,
                    State.Cycles,
                    executedInstructions);
            }
        ...
        private void ExecuteCachedFixedPlanGraphWalk(
        ...
        private bool FixedPlanRunLoopMatchesWindow(
            ...
            int instructionBase)
        {
            var instructionIndex = (int)slot.SinglePathLoopStartIndex;
            var checkedInstructions = 0;
            while (checkedInstructions++ < slot.InstructionCount)
            {
                ref var instruction = ref _fixedPlanRunInstructions![
                    instructionBase + instructionIndex];
                if (!FixedPlanRunInstructionMatchesWindow(in runWindow, in instruction))
                {
                    return false;
                }

                instructionIndex = instruction.TakenIndex != FixedPlanRunExitIndex
                    ? instruction.TakenIndex
                    : instruction.FallthroughIndex;
                if (instructionIndex == slot.SinglePathLoopStartIndex)
                {
                    return true;
                }

                if (instructionIndex == FixedPlanRunExitIndex)
                {
                    return false;
                }
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool IsFixedPlanRunEntryOpcode(ushort opcode, bool allowFastMemory)
        {
            var kind = M68kOpcodePlanTable.Kinds[opcode];
            return IsFixedPlanBatchOpcode(opcode, kind) ||
                IsShortConditionalBranch(opcode, kind) ||
                (allowFastMemory && IsFastMemoryRunKind(opcode, kind));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool IsShortConditionalBranch(ushort opcode, M68kOpcodePlanKind kind)
            => kind == M68kOpcodePlanKind.Branch &&
                (opcode & 0x00FF) != 0 &&
                ((opcode >> 8) & 0x0F) is >= 2 and <= 15;
            ...
            bool allowFastMemory)
        {
            ...
            return slot.Valid &&
                (!slot.ContainsFastMemory || allowFastMemory) &&
            ...
            bool allowFastMemory)
        {
            ...
            Span<uint> pendingProgramCounters = stackalloc uint[FixedPlanRunMaximumInstructions];
            Span<uint> fallthroughProgramCounters = stackalloc uint[FixedPlanRunMaximumInstructions];
            Span<uint> takenProgramCounters = stackalloc uint[FixedPlanRunMaximumInstructions];
            Span<bool> hasFallthrough = stackalloc bool[FixedPlanRunMaximumInstructions];
            Span<bool> hasTaken = stackalloc bool[FixedPlanRunMaximumInstructions];
            var pendingCount = 1;
            pendingProgramCounters[0] = entryProgramCounter;
            var containsFastMemory = false;
            var hasConditionalFlow = false;

            while (pendingCount != 0 && instructionCount < FixedPlanRunMaximumInstructions)
            {
                var programCounter = pendingProgramCounters[--pendingCount];
                if (FindFixedPlanRunInstructionIndex(
                    instructionBase,
                    instructionCount,
                    programCounter) != FixedPlanRunExitIndex)
                {
                    continue;
                }

                if (!TryDecodeFixedPlanRunInstruction(
                    entryProgramCounter,
                    entryWord0,
                    entryWord1,
                    programCounter,
                    ...
                    allowFastMemory,
                    out var instruction,
                    out var fallthroughProgramCounter,
                    out var instructionHasFallthrough,
                    out var takenProgramCounter,
                    out var instructionHasTaken))
                {
                    continue;
                }

                var currentIndex = instructionCount++;
                _fixedPlanRunInstructions![instructionBase + currentIndex] = instruction;
                fallthroughProgramCounters[currentIndex] = fallthroughProgramCounter;
                takenProgramCounters[currentIndex] = takenProgramCounter;
                hasFallthrough[currentIndex] = instructionHasFallthrough;
                hasTaken[currentIndex] = instructionHasTaken;
                containsFastMemory |= IsFastMemoryDataKind(instruction.Kind);
                hasConditionalFlow |= instruction.Kind is
                    M68kOpcodePlanKind.Branch or M68kOpcodePlanKind.Dbcc;

                QueueFixedPlanRunSuccessor(
                    fallthroughProgramCounter,
                    instructionHasFallthrough,
                    in runWindow,
                    allowFastMemory,
                    pendingProgramCounters,
                    ref pendingCount);
                QueueFixedPlanRunSuccessor(
                    takenProgramCounter,
                    instructionHasTaken,
            ...
            {
                ref var instruction = ref _fixedPlanRunInstructions![instructionBase + index];
                instruction.FallthroughIndex = hasFallthrough[index]
                    ? FindFixedPlanRunInstructionIndex(
                        instructionBase,
                        instructionCount,
                        fallthroughProgramCounters[index])
                    : FixedPlanRunExitIndex;
                instruction.TakenIndex = hasTaken[index]
                    ? FindFixedPlanRunInstructionIndex(
                        instructionBase,
                        instructionCount,
                        takenProgramCounters[index])
                    : FixedPlanRunExitIndex;
            }

            if (instructionCount == 0 ||
                !FixedPlanRunGraphHasCycle(instructionBase, instructionCount))
            {
                return false;
            }

            var singlePathLoopStartIndex = FixedPlanRunExitIndex;
            var singlePathLoop = !containsFastMemory &&
                !hasConditionalFlow &&
                TryGetSinglePathLoopStart(
                    instructionBase,
                    instructionCount,
                    out singlePathLoopStartIndex);

            var fetchWindow = runWindow.FetchWindow;
            slot.EntryProgramCounter = entryProgramCounter;
            slot.ContainsFastMemory = containsFastMemory;
            slot.HasConditionalFlow = hasConditionalFlow;
            slot.SinglePathLoop = singlePathLoop;
            ...
            slot.SinglePathLoopStartIndex = singlePathLoopStartIndex;
            slot.Valid = true;
            return true;
        }

        private bool TryDecodeFixedPlanRunInstruction(
            uint entryProgramCounter,
            ushort entryWord0,
            ushort entryWord1,
            uint programCounter,
            in M68kFixedPlanRunWindow runWindow,
            bool allowFastMemory,
            out M68000FixedPlanRunInstruction instruction,
            out uint fallthroughProgramCounter,
            out bool hasFallthrough,
            out uint takenProgramCounter,
            out bool hasTaken)
        {
            instruction = default;
            fallthroughProgramCounter = 0;
            hasFallthrough = false;
            takenProgramCounter = 0;
            hasTaken = false;
            if ((programCounter & 1) != 0 || !runWindow.ContainsWord(programCounter))
            {
                return false;
            }
            ...
            if (programCounter == unchecked(entryProgramCounter + 2) &&
                entryWord1 != runWindow.ReadWord(programCounter))
            {
                return false;
            }
            ...
            if (wordLength > 1)
            {
                ...
                extensionWord0 = programCounter == entryProgramCounter &&
                    entryWord1 == runWindow.ReadWord(extensionAddress)
                        ? entryWord1
                        : runWindow.ReadWord(extensionAddress);
            }
            ...
            var sequentialProgramCounter = unchecked(
                programCounter + ((uint)wordLength * 2u));
            if (kind == M68kOpcodePlanKind.ShortUnconditionalBranch)
            {
                takenProgramCounter = unchecked(
                    programCounter + 2u + (uint)(int)(sbyte)opcode);
                hasTaken = IsFixedPlanRunSuccessorFetchCovered(
                    in runWindow,
                    takenProgramCounter);
                if (!hasTaken)
                {
            ...
            else if (IsShortConditionalBranch(opcode, kind))
            {
                fallthroughProgramCounter = sequentialProgramCounter;
                takenProgramCounter = unchecked(
                    programCounter + 2u + (uint)(int)(sbyte)opcode);
                hasFallthrough = IsFixedPlanRunSuccessorFetchCovered(
                    in runWindow,
                    fallthroughProgramCounter);
                hasTaken = IsFixedPlanRunSuccessorFetchCovered(
                    in runWindow,
                    takenProgramCounter);
                if (!hasFallthrough || !hasTaken)
                {
                    return false;
                }
            }
            ...
            instruction.FallthroughIndex = FixedPlanRunExitIndex;
            instruction.TakenIndex = FixedPlanRunExitIndex;
            return true;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool IsFastMemoryDataKind(M68kOpcodePlanKind kind)
            => kind is
                M68kOpcodePlanKind.MoveLongPostincrementToData or
                M68kOpcodePlanKind.MoveLongDataToAbsoluteLong;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool IsFixedPlanRunSuccessorFetchCovered(
            in M68kFixedPlanRunWindow runWindow,
            uint programCounter)
            => (programCounter & 1) == 0 &&
                runWindow.ContainsWord(programCounter) &&
                runWindow.ContainsWord(unchecked(programCounter + 2));

        private static void QueueFixedPlanRunSuccessor(
            uint programCounter,
            bool hasSuccessor,
            in M68kFixedPlanRunWindow runWindow,
            bool allowFastMemory,
            Span<uint> pendingProgramCounters,
            ref int pendingCount)
        {
            if (!hasSuccessor || pendingCount >= pendingProgramCounters.Length)
            {
                return;
            }

            var opcode = runWindow.ReadWord(programCounter);
            if (!IsFixedPlanRunEntryOpcode(opcode, allowFastMemory))
            {
                return;
            }
            ...
            {
                if (pendingProgramCounters[index] == programCounter)
                {
                    return;
                }
            }

            pendingProgramCounters[pendingCount++] = programCounter;
        }

        private byte FindFixedPlanRunInstructionIndex(
            int instructionBase,
            int instructionCount,
            uint programCounter)
        {
            for (var index = 0; index < instructionCount; index++)
            {
                if (_fixedPlanRunInstructions![instructionBase + index].ProgramCounter ==
                    programCounter)
                {
                    return (byte)index;
                }
            ...
            return FixedPlanRunExitIndex;
        }

        private bool FixedPlanRunGraphHasCycle(int instructionBase, int instructionCount)
        {
            for (var index = 0; index < instructionCount; index++)
            {
                ref var instruction = ref _fixedPlanRunInstructions![instructionBase + index];
                if ((instruction.FallthroughIndex != FixedPlanRunExitIndex &&
                        FixedPlanRunHasPath(
                            instructionBase,
                            instructionCount,
                            instruction.FallthroughIndex,
        ...
        private bool FixedPlanRunHasPath(
            int instructionBase,
            int instructionCount,
            byte startIndex,
            byte targetIndex)
        {
            Span<byte> pending = stackalloc byte[FixedPlanRunMaximumInstructions];
            Span<bool> visited = stackalloc bool[FixedPlanRunMaximumInstructions];
            var pendingCount = 1;
            pending[0] = startIndex;
            while (pendingCount != 0)
            {
                var index = pending[--pendingCount];
                if (index == targetIndex)
                {
                    return true;
                }

                if (index >= instructionCount || visited[index])
                {
                    continue;
                }
                ...
                {
                    pending[pendingCount++] = instruction.FallthroughIndex;
                }

                if (instruction.TakenIndex != FixedPlanRunExitIndex &&
                    pendingCount < pending.Length)
                {
                    pending[pendingCount++] = instruction.TakenIndex;
                }
            }
        ...
        private bool TryGetSinglePathLoopStart(
            int instructionBase,
            int instructionCount,
            out byte loopStartIndex)
        {
            Span<bool> visited = stackalloc bool[FixedPlanRunMaximumInstructions];
            var index = (byte)0;
            for (var count = 0; count <= instructionCount; count++)
            {
                if (index == FixedPlanRunExitIndex || index >= instructionCount)
                {
                    break;
                }
                ...
                visited[index] = true;
                ref var instruction = ref _fixedPlanRunInstructions![instructionBase + index];
                var fallthrough = instruction.FallthroughIndex;
                var taken = instruction.TakenIndex;
                if (fallthrough != FixedPlanRunExitIndex && taken != FixedPlanRunExitIndex)
                {
                    break;
                }

                index = taken != FixedPlanRunExitIndex ? taken : fallthrough;
            }

            loopStartIndex = FixedPlanRunExitIndex;
            return false;
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private bool TryExecuteCachedFixedPlanGraphInstruction(
            ref M68000FixedBatchContext context,
            ref M68000FixedPlanRunRegisters registers,
            in M68000FixedPlanRunInstruction instruction,
            out byte nextInstructionIndex,
            out bool validateWindow)
        {
            validateWindow = true;
            if (instruction.Kind == M68kOpcodePlanKind.Branch)
            {
                return TryExecuteCachedFixedPlanConditionalBranch(
                    ref context,
                    ref registers,
                    in instruction,
                    out nextInstructionIndex);
            }

            if (instruction.Kind == M68kOpcodePlanKind.Dbcc)
            {
                return TryExecuteCachedFixedPlanDbra(
                    ref context,
                    ref registers,
                    in instruction,
                    out nextInstructionIndex);
            ...
            if (IsFastMemoryDataKind(instruction.Kind))
            {
                var executed = TryExecuteCachedFastMemoryRunInstruction(
                    ref context,
                    ref registers,
                    in instruction,
                    out validateWindow);
                nextInstructionIndex = instruction.FallthroughIndex;
                return executed;
            }
            ...
            ExecuteCachedFixedPlanRunInstruction(
                ...
                ref registers,
                in instruction);
            nextInstructionIndex = instruction.TakenIndex != FixedPlanRunExitIndex
                ? instruction.TakenIndex
                : instruction.FallthroughIndex;
            return true;
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private bool TryExecuteCachedFixedPlanConditionalBranch(
            ref M68000FixedBatchContext context,
            ref M68000FixedPlanRunRegisters registers,
            in M68000FixedPlanRunInstruction instruction,
            out byte nextInstructionIndex)
        {
            ...
            var taken = M68kIntegerSemantics.EvaluateCondition(
                registers.StatusRegister,
                plan.Condition);
            nextInstructionIndex = taken
                ? instruction.TakenIndex
                : instruction.FallthroughIndex;
            if (nextInstructionIndex == FixedPlanRunExitIndex)
            {
                return false;
            }
            ...
            if (taken)
            {
                ExecuteFixedBatchShortBranch(
                    ref context,
                    plan.Displacement,
                    retainStartedConditionalFallthrough: true);
                RecordPlannedFast(instruction.Kind);
                CompleteFixedBatchInstruction(
                    ref context,
                    PlannedRetireMode.PrefetchHandled);
                return true;
            }

            context.PendingInternalCycles += 4;
            CommitStartedFixedBatchConditionalBranchFallthroughPrefetch(
                ref context);
            CommitShortConditionalBranchFallthroughPrefetch(ref context);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private bool TryExecuteCachedFixedPlanDbra(
            ref M68000FixedBatchContext context,
            ref M68000FixedPlanRunRegisters registers,
            in M68000FixedPlanRunInstruction instruction,
            out byte nextInstructionIndex)
        {
            ...
            if ((target & 1) != 0)
            {
                nextInstructionIndex = FixedPlanRunExitIndex;
                return false;
            }
            ...
            if (taken)
            {
                ...
                context.InstructionInterruptSample = new M68000InterruptSample(
                    opcodeReadyCycle, context.ReadyBusTimingToken);
                RegisterDeferredInterruptSamples(in context);
            ...
            {
                _ = ReadFixedBatchPrefetchWord(
                    ref context,
                    target,
                    out _,
                    out _);
                var fallthrough = context.ProgramCounter;
                FlushFixedBatchPrefetch(ref context);
                context.PrefetchAddress = fallthrough;
                _ = TopUpFixedBatchPrefetchOne(ref context);
                var readyCycle = TopUpFixedBatchPrefetchOne(ref context);
                context.InstructionInterruptSample = new M68000InterruptSample(
                    readyCycle, context.ReadyBusTimingToken);
                RegisterDeferredInterruptSamples(in context);
                context.RetireBusCycle = Math.Max(context.RetireBusCycle, readyCycle);
                context.ExceptionEntryNotBeforeCycle = Math.Max(
                    context.ExceptionEntryNotBeforeCycle,
                    readyCycle);
                SetFixedBatchInstructionCycles(ref context, 14);
            }
        ...
        private void AdvanceCachedFixedPlanRunLoop(
            ...
            ref int executedInstructions)
        {
            var loopLength = GetSinglePathLoopLength(in slot, instructionBase);
            ...
            {
                var operationIndex = (int)slot.SinglePathLoopStartIndex;
                do
                {
                    ...
                    ExecuteFixedPlanRunArchitecturalOperation(
                        ref registers,
                        in operation);
                    operationIndex = operation.TakenIndex != FixedPlanRunExitIndex
                        ? operation.TakenIndex
                        : operation.FallthroughIndex;
                }
                while (operationIndex != slot.SinglePathLoopStartIndex &&
        ...
        private int GetSinglePathLoopLength(
            in M68000FixedPlanRunSlot slot,
            int instructionBase)
        {
            var length = 0;
            var instructionIndex = slot.SinglePathLoopStartIndex;
            do
            {
                if (instructionIndex == FixedPlanRunExitIndex ||
                    instructionIndex >= slot.InstructionCount ||
                    length >= slot.InstructionCount)
                {
                    return 0;
                }
            ...
            while (instructionIndex != slot.SinglePathLoopStartIndex);
        ...
        private static bool IsSteadyFixedPlanRunLoop(
            ...
            => after.ProgramCounter == before.ProgramCounter &&
                ...
                after.PendingPrefetchAddressPhaseIssued ==
                    before.PendingPrefetchAddressPhaseIssued &&
        ...
        private M68000FixedBatchContext CaptureFixedBatchContext()
        {
            ...
            return new M68000FixedBatchContext
            {
                ...
                ReadyBusTimingToken = _cpuBusReadyTimingToken,
                ...
                PrefetchTimingToken0 = _prefetchTimingToken0,
                PrefetchTimingToken1 = _prefetchTimingToken1,
                ...
                PendingPrefetchBlocksInstructionEntry =
                    _pendingPrefetchBlocksInstructionEntry,
                PendingPrefetchAddressPhaseIssued =
                    _pendingPrefetchAddressPhaseIssued,
                ...
                InstructionInterruptSample = _instructionInterruptSample,
                LastInterruptSample = _lastInterruptSample,
        ...
        private void CommitFixedBatchContext(in M68000FixedBatchContext context)
        {
            ...
            _cpuBusReadyTimingToken = context.ReadyBusTimingToken;
            ...
            _prefetchTimingToken0 = context.PrefetchTimingToken0;
            _prefetchTimingToken1 = context.PrefetchTimingToken1;
            ...
            _pendingPrefetchBlocksInstructionEntry =
                context.PendingPrefetchBlocksInstructionEntry;
            _pendingPrefetchAddressPhaseIssued =
                context.PendingPrefetchAddressPhaseIssued;
            ...
            _instructionInterruptSample = context.InstructionInterruptSample;
            _lastInterruptSample = context.LastInterruptSample;
        ...
        private ushort ConsumeFixedBatchPrefetchedWord(ref M68000FixedBatchContext context)
        {
            ...
            if (context.PrefetchCount > 1)
            {
                ...
                context.PrefetchTimingToken0 = context.PrefetchTimingToken1;
            ...
            {
                context.PrefetchCount = 0;
                context.PrefetchTimingToken0 = 0;
                context.PrefetchDeferredBatchEligible0 = false;
            }

            context.PrefetchTimingToken1 = 0;
        ...
        private void ExecuteFixedBatchShortBranch(
            ref M68000FixedBatchContext context,
            int displacement,
            bool retainStartedConditionalFallthrough = false)
        {
            var branchBase = context.ProgramCounter;
            context.PendingInternalCycles += 2;
            if (retainStartedConditionalFallthrough)
            {
                CommitStartedFixedBatchConditionalBranchFallthroughPrefetch(
                    ref context);
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void CommitStartedFixedBatchConditionalBranchFallthroughPrefetch(
            ref M68000FixedBatchContext context)
        {
            if (_m68000BusCycleTiming == null ||
                !context.HasPendingPrefetch ||
                !context.PendingPrefetchAddressPhaseIssued ||
                context.PrefetchCount >= 2 ||
                context.PrefetchAddress != context.ProgramCounter ||
            ...
            var pendingInternalCycles = context.PendingInternalCycles;
            context.PendingInternalCycles = 0;
            try
            {
                var readyCycle = TopUpFixedBatchPrefetchOne(ref context);
                context.Cycles = Math.Max(context.Cycles, readyCycle);
                context.RetireBusCycle = Math.Max(context.RetireBusCycle, readyCycle);
            }
            finally
            {
                context.PendingInternalCycles = pendingInternalCycles;
            }
        }
            ...
            ref M68000FixedBatchContext context)
        {
            ...
            {
                context.PrefetchAddress = context.ProgramCounter;
                var selectedReadyCycle = TopUpFixedBatchPrefetchOne(ref context);
                context.RetireBusCycle = Math.Max(
                    context.RetireBusCycle,
                    selectedReadyCycle);
            }
            ...
            if (context.PrefetchCount > 0 &&
                context.PrefetchAddress == context.ProgramCounter)
            {
                var selectedReadyCycle = context.PrefetchReadyCycle0;
                context.Cycles = Math.Max(context.Cycles, selectedReadyCycle);
                context.RetireBusCycle = Math.Max(
                    context.RetireBusCycle,
                    selectedReadyCycle);
                if (context.InstructionInterruptSampleCycle == long.MinValue)
                {
                    var timingToken = context.Cycles == selectedReadyCycle
                        ? context.PrefetchTimingToken0
                        : 0;
                    context.InstructionInterruptSample = new M68000InterruptSample(
                        context.Cycles,
                        timingToken);
            ...
            {
                var tailReadyCycle = TopUpFixedBatchPrefetchOne(ref context);
                context.RetireBusCycle = Math.Max(
                    context.RetireBusCycle,
                    tailReadyCycle);
            }
            ...
            uint target)
        {
            ...
            var targetOpcodeReadyCycle =
                TopUpFixedBatchPrefetchOne(ref context);
            context.InstructionInterruptSample = new M68000InterruptSample(
                targetOpcodeReadyCycle, context.ReadyBusTimingToken);
            RegisterDeferredInterruptSamples(in context);
        ...
        private static void FlushFixedBatchPrefetch(ref M68000FixedBatchContext context)
        {
            ...
            context.PendingPrefetchBlocksInstructionEntry = false;
            context.PendingPrefetchAddressPhaseIssued = false;
            context.PrefetchTimingToken0 = 0;
            context.PrefetchTimingToken1 = 0;
            context.PrefetchDeferredBatchEligible0 = false;
            context.PrefetchDeferredBatchEligible1 = false;
            ...
            PlannedRetireMode retireMode)
        {
            ...
            {
                context.InstructionInterruptSample = new M68000InterruptSample(
                    context.ReadyBusCycle,
                    context.ReadyBusTimingToken);
                RegisterDeferredInterruptSamples(in context);
            }
            ...
            ResolveDeferredInterruptSamples(ref context);
            context.LastInterruptSample = context.InstructionInterruptSample;
            RegisterDeferredInterruptSamples(in context);
        ...
        private void TopUpFixedBatchPrefetchAtRetirement(ref M68000FixedBatchContext context)
        {
            ...
            // When an instruction drains both queue words, CE000 selects the
            // next opcode before it performs the following IRC transfer.  The
            // IPL poll belongs to that selected opcode phase; the second word
            // remains an irrevocable physical tail but must not move the poll.
            var sampleAfterFirstRefill =
                context.PrefetchCount == 0 &&
                context.InstructionInterruptSampleCycle == long.MinValue;
			...
			while (context.PrefetchCount < 2)
			{
				...
				var earliestCycle = context.HasPendingPrefetch
					...
					: Math.Max(
						context.Cycles + busCycleStartDelay,
						context.NextBusCycle);
				if (!context.PendingPrefetchAddressPhaseIssued &&
					earliestCycle + 2 > context.InstructionFloor)
                {
                    if (!context.HasPendingPrefetch)
                    {
                ...
                var readyCycle = TopUpFixedBatchPrefetchOne(ref context);
                context.RetireBusCycle = Math.Max(context.RetireBusCycle, readyCycle);
                if (sampleAfterFirstRefill && context.PrefetchCount == 1)
                {
                    context.InstructionInterruptSample = new M68000InterruptSample(
                        readyCycle,
                        context.ReadyBusTimingToken);
                    RegisterDeferredInterruptSamples(in context);
                    sampleAfterFirstRefill = false;
                }
            }
        }
            ...
            out long requestedCycle)
        {
            ...
            var addressPhaseAlreadyIssued = false;
            var issuedAddressPhaseEarliestCycle = 0L;
            if (context.HasPendingPrefetch)
            {
                ...
                addressPhaseAlreadyIssued =
                    context.PendingPrefetchAddressPhaseIssued;
                issuedAddressPhaseEarliestCycle =
                    context.PendingPrefetchEarliestCycle;
                context.HasPendingPrefetch = false;
                context.PendingPrefetchAddress = 0;
                context.PendingPrefetchEarliestCycle = 0;
            ...
            if (slot == 0)
            {
                ...
                context.PrefetchTimingToken0 = context.ReadyBusTimingToken;
            ...
            else
            {
                ...
                context.PrefetchTimingToken1 = context.ReadyBusTimingToken;
        ...
        {
            var busAddress = GetCpuBusAddress(address);
            var cycle = addressPhaseAlreadyIssued
                ? Math.Max(
                    issuedAddressPhaseEarliestCycle,
                    context.NextBusCycle)
                : context.Cycles;
            if (!addressPhaseAlreadyIssued && context.PendingInternalCycles != 0)
            {
                cycle += context.PendingInternalCycles;
                context.PendingInternalCycles = 0;
            }

            if (!addressPhaseAlreadyIssued && _m68000BusCycleTiming != null)
            {
                cycle += _m68000BusCycleTiming.M68000BusCycleStartDelay;
            }
            ...
            context.ReadyBusCycle = timing.ReadyCycle;
            if (deferredEligible &&
                _deferredCpuInstructionTiming is { IsDeferredCpuBusBatchActive: true })
            {
                context.ReadyBusTimingToken =
                    _deferredCpuInstructionTiming.CaptureDeferredCpuInstructionFetchTimingToken();
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private bool CanEnterDeferredCpuBusBatchInstruction()
        {
            if (_deferredCpuInstructionTiming?.RequiresControlFlowBatchBoundary == true &&
                (IsMemoryFormMoveOpcode(_prefetchWord0) ||
                    IsDeferredCpuBatchControlFlowOpcode(_prefetchWord0)))
            {
                return false;
            ...
            {
                return _instructionFetchWindowBus != null &&
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool IsMemoryFormMoveOpcode(ushort opcode)
        {
            var sizeNibble = opcode >> 12;
            if (sizeNibble is < 1 or > 3)
            {
                return false;
            }

            var sourceMode = (opcode >> 3) & 0x07;
            var destinationMode = (opcode >> 6) & 0x07;
            return sourceMode >= 2 || destinationMode >= 2;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private bool EnsureDeferredCpuBusBatchPrefetchReady()
        {
            // Batch admission may make a missing opcode ready, but it must not
            // publish a cancellable successor before the instruction chooses
            // its data/prefetch order. A valid head needs no extra bus work.
            if (_prefetchCount == 0 &&
                _hasPendingPrefetch &&
                _pendingPrefetchAddress == State.ProgramCounter)
            {
                ...
                _prefetchAddress = State.ProgramCounter;
                FullPrefetch(State.ProgramCounter);
            }
        ...
        private int ExecuteInstructionBody(long startCycles)
        {
            ...
            var consumedFromFullPlannedQueue =
                _opcodePlanDispatch != M68kOpcodePlanDispatch.Scalar &&
                _prefetchCount == 2 &&
                _prefetchAddress == instructionPc;
            var opcode = consumedFromFullPlannedQueue
                ? ConsumeSequentialPlannedOpcodeWord()
                : FetchOpcodeWord();
            ...
            if ((opcode & 0xF000) == 0xF000)
            {
                if (opcode == 0xFF00)
                {
                    ...
                    var gateway = _bus.InvokeHostGateway(GetCpuBusAddress(instructionPc), token, State);
                    if (gateway.Handled)
                    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private PlannedRetireMode TryExecutePlannedInstruction(ushort opcode, uint instructionPc)
        {
            return _opcodePlanDispatch switch
            {
                ...
                _ => PlannedRetireMode.Unsupported
            };
        }
        ...
        private PlannedRetireMode TryExecutePlannedKind(ushort opcode, uint instructionPc, M68kOpcodePlanKind kind)
        {
            switch (kind)
            {
                ...
                case M68kOpcodePlanKind.QuickRegister:
                    ...
                    return PlannedRetireMode.SequentialOneWordRefill;
                case M68kOpcodePlanKind.QuickLongDataRegister:
                    ExecutePlannedQuickLongDataRegister(opcode);
                    RecordPlannedFast(kind);
                    return PlannedRetireMode.SequentialOneWordRefill;
                ...
                case M68kOpcodePlanKind.MoveLongDataToPostincrement:
                    ...
                    RecordPlannedFast(kind);
                    return PlannedRetireMode.General;
                case M68kOpcodePlanKind.MoveLongDataToAbsoluteLong:
                    ExecutePlannedMoveLongDataToAbsoluteLong(opcode);
                    RecordPlannedFast(kind);
                    return PlannedRetireMode.General;
        ...
        private PlannedRetireMode TryExecutePackedPlan(ushort opcode, uint instructionPc, in M68kPackedOpcodePlan plan)
        {
            ...
            switch (kind)
            {
                ...
                case M68kOpcodePlanKind.QuickRegister:
                    ...
                    return PlannedRetireMode.SequentialOneWordRefill;
                case M68kOpcodePlanKind.QuickLongDataRegister:
                    ExecutePackedQuickLongDataRegister(in plan);
                    RecordPlannedFast(kind);
                    return PlannedRetireMode.SequentialOneWordRefill;
                case M68kOpcodePlanKind.Move:
                ...
                case M68kOpcodePlanKind.MoveLongDataToPostincrement:
                    ...
                    return PlannedRetireMode.General;
                case M68kOpcodePlanKind.MoveLongDataToAbsoluteLong:
                    ExecutePackedMoveLongDataToAbsoluteLong(in plan);
                    RecordPlannedFast(kind);
                    return PlannedRetireMode.General;
                case M68kOpcodePlanKind.Immediate:
        ...
        private void ExecutePlannedBranch(ushort opcode, uint instructionPc)
        {
			...
			if (condition >= 2 && !extensionDisplacement)
			{
				CommitStartedConditionalBranchFallthroughPrefetch();
			}
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ExecutePlannedShortUnconditionalBranch(ushort opcode, uint instructionPc)
            => ExecuteShortUnconditionalBranch(opcode, unchecked((sbyte)opcode), instructionPc);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ExecutePackedShortUnconditionalBranch(
            ushort opcode,
            in M68kPackedOpcodePlan plan,
            uint instructionPc)
            => ExecuteShortUnconditionalBranch(opcode, plan.Displacement, instructionPc);
        ...
        private void ExecuteShortUnconditionalBranch(ushort opcode, int displacement, uint instructionPc)
        {
            ...
            if (_instructionFrequency.Enabled)
            {
                _instructionFrequency.RecordTakenBranch(instructionPc, opcode, target, 2);
            }
        ...
        private void ExecutePlannedDbcc(ushort opcode)
        {
            ...
            AddPendingInternalCycles(conditionTrue ? 4 : 2);
            _consumeWithoutPrefetch = !conditionTrue;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ExecutePlannedQuickLongDataRegister(ushort opcode)
        {
            var count = (opcode >> 9) & 7;
            ExecuteQuickLongDataRegister(
                opcode & 7,
                count == 0 ? 8u : (uint)count,
                (opcode & 0x0100) != 0);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ExecutePackedQuickLongDataRegister(in M68kPackedOpcodePlan plan)
            => ExecuteQuickLongDataRegister(plan.DestinationRegister, plan.QuickValue, plan.Variant != 0);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ExecutePlannedMoveLongDataToAbsoluteLong(ushort opcode)
            => ExecuteMoveLongDataToAbsoluteLong(opcode & 7);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ExecutePackedMoveLongDataToAbsoluteLong(in M68kPackedOpcodePlan plan)
            => ExecuteMoveLongDataToAbsoluteLong(plan.SourceRegister);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ExecuteMoveLongDataToAbsoluteLong(int sourceRegister)
        {
            var destinationAddress = FetchLongWithoutPrefetchTopUp();
            var value = State.D[sourceRegister];
            _dataAccessStackedProgramCounter = State.ProgramCounter;
            State.SetNegativeZeroLong(value);
            State.SetFlag(M68kCpuState.Overflow, false);
        ...
        private void ExecutePlannedImmediate(ushort opcode)
        {
            ...
            var operation = opcode & 0xFF00;
            var sampleIplBetweenLongWords =
                mode == 0 &&
                size == M68kOperandSize.Long &&
                operation is 0x0400 or 0x0600;
            var immediate = FetchImmediate(size, sampleIplBetweenLongWords);
            if (mode == 0 && !sampleIplBetweenLongWords)
            {
                SampleM68000IplAtQueuedOpcode();
            }
            ...
            var prefetchBeforeWriteback = writesEffectiveAddress && !destinationEa.IsRegister;
            var destination = ReadPlannedEaValue(in destinationEa);
            if (prefetchBeforeWriteback)
            {
                PrefetchFallthroughBeforeMemoryWriteback();
            }
            ...
            if (!_useM68020BriefIndexedAddressing && mode == 0)
            {
                CommitRequiredSequentialFinalPrefetch();
            }
        ...
        {
            var register = opcode & 7;
            var immediate = FetchImmediate(M68kOperandSize.Byte);
            SampleM68000IplAtQueuedOpcode();
            var result = (State.D[register] & 0xFF) & immediate;
        ...
        private void ExecutePackedBranch(in M68kPackedOpcodePlan plan, uint instructionPc)
        {
            ...
            AddPendingInternalCycles(taken ? 2 : 4);
			if (plan.Condition >= 2 && !plan.ExtensionDisplacement)
			{
				CommitStartedConditionalBranchFallthroughPrefetch();
			}
            _consumeWithoutPrefetch = taken;
        ...
        private void ExecutePackedDbcc(ushort opcode, in M68kPackedOpcodePlan plan)
        {
            ...
            var branchBase = State.ProgramCounter;
            var conditionTrue = CheckCondition(plan.Condition);
            AddPendingInternalCycles(conditionTrue ? 4 : 2);
            _consumeWithoutPrefetch = !conditionTrue;
            var displacement = unchecked((short)FetchWord());
            _consumeWithoutPrefetch = false;
        ...
        private void ExecutePackedMove(in M68kPackedOpcodePlan plan)
        {
            ...
            if (!moveWritesMemory)
            {
                PrefetchFallthroughAfterMoveSourceRead(
                    sampleIplAfterDataPhase:
                        plan.Size == M68kOperandSize.Word &&
                        plan.SourceMode == 2 &&
        ...
        private void ExecutePackedImmediate(ushort opcode, in M68kPackedOpcodePlan plan)
        {
            ...
            var sampleIplBetweenLongWords =
                plan.DestinationMode == 0 &&
                plan.Size == M68kOperandSize.Long &&
                plan.Variant is 2 or 3;
            var immediate = FetchImmediate(plan.Size, sampleIplBetweenLongWords);
            if (plan.DestinationMode == 0 && !sampleIplBetweenLongWords)
            {
                SampleM68000IplAtQueuedOpcode();
            }
            ...
            var destination = ReadPlannedEaValue(in destinationEa);
            if (writesEffectiveAddress && !destinationEa.IsRegister)
            {
                PrefetchFallthroughBeforeMemoryWriteback();
            }
            ...
            if (!_useM68020BriefIndexedAddressing && plan.DestinationMode == 0)
            {
                CommitRequiredSequentialFinalPrefetch();
            }
            ...
            int addressErrorStackedProgramCounterOffset = 0)
        {
            switch (mode)
            {
                ...
                case 6:
                {
                    ...
                    return PlannedEaOperand.Memory(
                        CalculateBriefIndexedAddress(State.A[register], extension),
                        size,
                        GetEaOperandCycles(mode, register, size),
                        unchecked((uint)(extensionAddress +
                            (write && size == M68kOperandSize.Long ? 2u : 0u) +
        ...
        private PlannedEaOperand ResolvePlannedEaWithoutPrefetchTopUp(
            int mode,
            int register,
            M68kOperandSize size,
            bool write = false,
            ...
            int addressErrorStackedProgramCounterOffset = 0)
        {
            var previous = _consumeWithoutPrefetch;
            _consumeWithoutPrefetch = true;
            try
            {
                return ResolvePlannedEa(
                    mode,
                    register,
                    size,
                    write,
                    completeWordPostIncrementBeforeRead,
                    addressErrorStackedProgramCounterOffset);
            }
            finally
            {
                _consumeWithoutPrefetch = previous;
            }
        }
            ...
            int addressErrorStackedProgramCounterOffset = 0)
        {
            switch (register)
            {
                ...
                case 3:
                {
                    ...
                    var extension = FetchWord();
                    return PlannedEaOperand.Memory(
                        CalculateBriefIndexedAddress(extensionAddress, extension),
                        size,
                        GetEaOperandCycles(7, register, size),
                        extensionAddress,
                        readFaultAccessKind: M68kBusAccessKind.CpuInstructionFetch);
        ...
        private static int GetMoveLongDestinationWriteFaultCycles(int successfulCycles, int destinationMode, int destinationRegister)
        {
            ...
            if (destinationMode == 7 && destinationRegister == 1)
            {
                return Math.Max(16, successfulCycles - 8);
            }
        ...
        M68000PipelineState IM68000PipelineStateTransfer.ExportM68000PipelineState()
            => new M68000PipelineState(
                ...
                _prefetchCompletedCycle1,
                _prefetchDeferredCpuBusBatchEligible0,
                _prefetchDeferredCpuBusBatchEligible1,
                _prefetchCount,
                _consumeWithoutPrefetch,
                _skipRetirePrefetchTopUp,
                _cpuBusCycle,
                _cpuBusReadyCycle,
                _cpuRetireBusCycle,
                ...
                _lastInterruptSampleCycle)
            {
                LastBusReadyTimingToken = _cpuBusReadyTimingToken,
                LastInterruptSampleTimingToken = _lastInterruptSample.DeferredFetchToken,
                PendingPrefetchAddressPhaseIssued =
                    _pendingPrefetchAddressPhaseIssued
            };
        ...
        {
            _prefetchAddress = state.PrefetchAddress;
            _prefetchWord0 = state.Word0;
            ...
            _consumeWithoutPrefetch = state.ConsumeWithoutPrefetch;
            _skipRetirePrefetchTopUp = state.SkipRetirePrefetchTopUp;
            _cpuBusCycle = state.NextBusTransferCycle;
            _cpuBusReadyCycle = state.LastBusReadyCycle;
            _cpuBusReadyTimingToken = state.LastBusReadyTimingToken;
            ...
            _pendingPrefetchAddressPhaseIssued =
                state.PendingPrefetchAddressPhaseIssued;
            ...
            _lastInterruptSample = new M68000InterruptSample(
                state.LastInterruptSampleCycle,
                state.LastInterruptSampleTimingToken);
            RegisterDeferredInterruptSamples();
        }
        ...
        private void PerformInterruptAcknowledge(int level)
        {
            // IACK uses the CPU-space/autovector handshake, not an Agnus data
            // slot. The preceding stack write has completed; adding the chip
            // bus's address-to-grant phase here would charge it a second time.
            var cycle = Math.Max(State.Cycles, _cpuBusReadyCycle);
            var requestedCycle = cycle;
            cycle += 4;
            CompleteCpuBusAccess(cycle);
            RecordCpuBusPhase(
                GetCpuBusAddress(0x00FF_FFF1u | ((uint)(level & 7) << 1)),
                M68kOperandSize.Word,
                requestedCycle,
                cycle,
                M68kBusAccessKind.CpuInterruptAcknowledge,
                isWrite: false);
        }

        private void PrefetchInterruptTarget(uint target)
        {
            ...
            // The exception microsequence separates IR and IRC by two internal
            // clocks. They advance CPU control without reserving an Agnus slot;
            // the subsequent IRC read still begins with its ordinary address
            // phase.
        ...
        private bool DecodeBranch(ushort opcode, uint instructionPc)
        {
			...
			var dependentWordBsrInternalPhase =
				condition == 1 && displacement == 0 && !_useM68020BriefIndexedAddressing;
			if (!dependentWordBsrInternalPhase)
			{
                AddPendingInternalCycles(taken ? 2 : 4);
			}
			if (condition >= 2 && displacement != 0)
			{
				CommitStartedConditionalBranchFallthroughPrefetch();
			}
            _consumeWithoutPrefetch = taken;
			...
			if (dependentWordBsrInternalPhase)
			{
				// The MC68000/010 word displacement is an operand dependency.
				// Its IRC transfer must finish before BSR's two internal CPU
				// cycles; otherwise a future-ready word absorbs that phase.
				AddPendingInternalCycles(2);
			}
            ...
            if (condition == 1)
            {
                ...
                if ((target & 1) != 0)
                {
                    ...
                    PushBsrReturnAddress(State.ProgramCounter);
                ...
                BranchToAndRefillTarget(
                    target,
                    target,
                    interruptibleTargetFetch: false,
                    branchTransitionKind: M68kBranchInterruptTransitionKind.None);
                return true;
            }
        ...
        private bool DecodeLine0(ushort opcode, uint instructionPc)
        {
            ...
            var prefetchBeforeWriteback = writesEffectiveAddress && !ea.IsRegister;
        ...
        {
            // The unary group (NEGX, CLR, NEG, NOT, TST) dominates line 4 in
            // real code, while every other form in this method either uses
            // size bits 0b11 or a 0x41/0x48/0x4E prefix. Dispatching it first
            // keeps the hot path off the rare control/supervisor chain below
            // without changing which form any opcode selects.
            var unaryGroup = opcode & 0xFF00;
            if (((opcode >> 6) & 3) != 3 &&
                unaryGroup is 0x4000 or 0x4200 or 0x4400 or 0x4600 or 0x4A00)
            {
                return ExecuteLine4Unary(opcode, instructionPc, unaryGroup);
            }
            ...
            {
                return ExecuteLine4Unary(opcode, instructionPc, unary);
            }
        ...
        private bool ExecuteLine4Unary(ushort opcode, uint instructionPc, int unary)
        {
            ...
            var isMemoryTstLong =
                unary == 0x4A00 && size == M68kOperandSize.Long;
            var value = isMemoryTstLong
                ? ea.ReadTstLongWithIplSampleBetweenWords()
                : ea.Read();
            switch (unary)
            {
                case 0x4000:
                ...
                case 0x4600:
                    ...
                    break;
                case 0x4A00:
                    State.SetSizedLogicFlags(value, size);
                    if (isMemoryTstLong)
                    {
                        CommitRequiredSequentialFinalPrefetch(
                            requireTransferRetirementEdge: true);
                    }
                    break;
            }
        ...
        private bool DecodeArithmetic(ushort opcode)
        {
            ...
            if (line == 0xC && opmode == 3)
            {
                ...
                State.SetFlag(M68kCpuState.Overflow, false);
                State.SetFlag(M68kCpuState.Carry, false);
                AddInternalNoBusInstructionCycles(
                    GetMultiplyCycles(sourceEa.EaCycles, source, signed: false),
                    GetMultiplyCoreCycles(source, signed: false),
                    M68kInternalNoBusWindowKind.Multiply);
                return true;
            }
            ...
            if (line == 0xC && opmode == 7)
            {
                ...
                State.SetFlag(M68kCpuState.Carry, false);
                AddInternalNoBusInstructionCycles(
                    GetMultiplyCycles(sourceEa.EaCycles, (ushort)source, signed: true),
                    GetMultiplyCoreCycles((ushort)source, signed: true),
                    M68kInternalNoBusWindowKind.Multiply);
                return true;
            }
            ...
            if (line == 0x8 && (opmode == 3 || opmode == 7))
            {
                ...
                AddInternalNoBusInstructionCycles(
                    4 + GetDivideCycles(sourceEa.EaCycles, dividend, (ushort)divisor, signed: opmode == 7),
                    4 + GetDivideCoreCycles(dividend, (ushort)divisor, signed: opmode == 7),
                    M68kInternalNoBusWindowKind.Divide);
                return true;
            }
            ...
            if (mode == 7 && eaReg == 4 && !writesEffectiveAddress)
            {
                SampleM68000IplAtQueuedOpcode();
            }
        ...
        private void DecodeMovem(ushort opcode)
        {
            ...
            if (!directionMemoryToRegisters && mode == 4)
            {
                ...
                CommitMovemRetirementPrefetchIfRequestPhaseStarted();
                return;
            }
            ...
            if (directionMemoryToRegisters && !_useM68020BriefIndexedAddressing)
            {
                // The 68000/68010 read sequence finishes with one ignored word
                // at the next address, even for an empty mask. It is a real
                // data-bus transfer, before postincrement writeback and the
                // final prefetch, not additional internal execution time.
                _ = ReadWord(current);
            }
            ...
            if (directionMemoryToRegisters && !_useM68020BriefIndexedAddressing)
            {
                // The final instruction word follows the ignored data read.
                // It is required by MOVEM itself, even when contention has
                // already carried the bus beyond the nominal retire floor.
            ...
            int addressErrorStackedProgramCounterOffset = 0)
        {
            switch (mode)
            {
                ...
                case 6:
                {
                    ...
                    return EaOperand.Memory(
                        this,
                        CalculateBriefIndexedAddress(State.A[reg], extension),
                        size,
                        GetEaOperandCycles(mode, reg, size),
                        unchecked((uint)(extensionAddress +
                            (write && size == M68kOperandSize.Long ? 2u : 0u) +
        ...
        private EaOperand ResolveEaWithoutPrefetchTopUp(
            int mode,
            int reg,
            M68kOperandSize size,
            bool write = false,
        ...
        private uint CalculateBriefIndexedAddress(uint baseAddress, ushort extension)
        {
            if (_useM68020BriefIndexedAddressing &&
                ...
                M68kIntegerSemantics.TryCalculateM68020BriefIndexedAddress(
                    ...
                    State.D,
                    State.A,
                    out var scaledAddress))
            {
                return scaledAddress;
            }

            return M68kIntegerSemantics.CalculateM68000BriefIndexedAddress(
                baseAddress,
                extension,
                State.D,
                State.A);
        ...
        private uint FetchImmediate(
            M68kOperandSize size,
            bool sampleIplBetweenLongWords = false)
        {
            if (size == M68kOperandSize.Byte)
            {
                return (uint)(FetchWord() & 0xFF);
            }
            ...
            var high = FetchWord();
            if (sampleIplBetweenLongWords)
            {
                // Long SUBI/ADDI poll the current IPL pins after the first
                // immediate micro-operation refills IRC with the low word.
                SampleM68000IplAtCurrentBusPhase();
            }
            var low = FetchWord();
            return ((uint)high << 16) | low;
        }

        private uint FetchImmediateWithoutPrefetchTopUp(M68kOperandSize size)
        {
            var previous = _consumeWithoutPrefetch;
            _consumeWithoutPrefetch = true;
            try
            {
                return FetchImmediate(size);
            }
            finally
            {
                _consumeWithoutPrefetch = previous;
        ...
        private uint FetchLongWithoutPrefetchTopUp()
        {
            var previous = _consumeWithoutPrefetch;
            _consumeWithoutPrefetch = true;
            try
        ...
        private ushort FetchOpcodeWord()
        {
            ...
            {
                FullPrefetch(address);
            }

            return ConsumePrefetchedWord();
        }
        ...
        protected ushort FetchWord()
        {
            ...
            var value = ConsumePrefetchedWord();
            if (!_consumeWithoutPrefetch)
            {
                TopUpPrefetchOne();
            }
        ...
        private void BranchToAndPrimeTargetOpcode(uint target, uint stackedProgramCounter)
        {
            if ((target & 1) != 0)
            {
                _dataAccessStackedProgramCounter = stackedProgramCounter;
            ...
            _prefetchAddress = target;
            var readyCycle = TopUpPrefetchOne();
            _instructionInterruptSample = new M68000InterruptSample(
                readyCycle, _cpuBusReadyTimingToken);
            RegisterDeferredInterruptSamples();
            _skipRetirePrefetchTopUp = true;
        }
        ...
        private void TransitionTakenDbccTarget(uint target, uint stackedProgramCounter)
        {
            ...
            _instructionInterruptSample = new M68000InterruptSample(
                targetOpcodeReadyCycle, _cpuBusReadyTimingToken);
            RegisterDeferredInterruptSamples();
            ...
            _cpuRetireBusCycle = Math.Max(
                _cpuRetireBusCycle,
                targetExtensionReadyCycle);
            // IPL is sampled after the target opcode read. An interrupt must
            // retain the committed extension's completion, but DBcc has no
            // further tail after that read. Exception entry supplies its own
            // six internal setup cycles; charging them here would repeat them.
        ...
        private void ValidateBranchTarget(uint target, uint stackedProgramCounter)
        {
            if ((target & 1) == 0)
            {
                return;
        ...
        private void PushBsrReturnAddress(uint value)
        {
            if (_useM68020BriefIndexedAddressing)
            {
                PushSubroutineReturnAddress(value);
                return;
            }

            // The MC68000/010 BSR stack transfer is high-word first, unlike
            // a generic longword predecrement write.
            State.SetActiveStackPointer(State.A[7] - 4);
        ...
        private void UsePrefetchedInstructionWordForAddressErrorFrame()
        {
            if (_prefetchCount > 0)
            {
                _addressErrorInstructionWord = _prefetchWord0;
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private ushort ConsumePrefetchedWord()
        {
            if (_prefetchCount == 0)
            {
                FillPrefetchSlot(State.ProgramCounter);
            }

            var value = _prefetchWord0;
            var completedCycle = _prefetchCompletedCycle0;
            if (_prefetchTimingToken0 >= _instructionDeferredCpuTimingDependencyToken)
            {
                _instructionDeferredCpuTimingDependencyToken = _prefetchTimingToken0;
                _instructionDeferredCpuTimingDependencyVirtualReadyCycle = completedCycle;
            }
            var address = _prefetchAddress;
            State.ProgramCounter = unchecked(address + 2);
            if (_pendingInternalCycles != 0)
            {
                State.Cycles += _pendingInternalCycles;
                _pendingInternalCycles = 0;
            }
            ...
            if (_prefetchCount > 1)
            {
                _prefetchAddress = unchecked(address + 2);
                _prefetchWord0 = _prefetchWord1;
                _prefetchCompletedCycle0 = _prefetchCompletedCycle1;
                _prefetchTimingToken0 = _prefetchTimingToken1;
                _prefetchDeferredCpuBusBatchEligible0 = _prefetchDeferredCpuBusBatchEligible1;
                _prefetchCount = 1;
            }
            else
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private ushort ConsumeSequentialPlannedOpcodeWord()
        {
            System.Diagnostics.Debug.Assert(_prefetchCount == 2);
        ...
        private long FullPrefetch(uint address)
        {
            if (_hasPendingPrefetch &&
                _pendingPrefetchAddressPhaseIssued &&
                _pendingPrefetchAddress != address)
            {
                CommitIssuedPendingPrefetch();
            }

            var publicationContext = _hasPendingPrefetch &&
                _pendingPrefetchAddress == address
                    ? _pendingPrefetchPublicationContext
                    : CaptureInstructionFetchPublicationContext();
            ...
            _prefetchDeferredCpuBusBatchEligible1 = false;
            TopUpPrefetchOne();
            return TopUpPrefetchOne(
                out _,
                M68kInstructionFetchPublicationPhase.Required,
                long.MinValue,
                publicationContext with
                {
                    PredecessorToken = _prefetchTimingToken0,
                    RequiresIrcGap = _prefetchTimingToken0 != 0
                });
            ...
            long issuedAddressPhaseEarliestCycle = 0)
        {
            ...
            if (_hasPendingPrefetch)
            {
                ...
                addressPhaseAlreadyIssued |=
                    _pendingPrefetchAddressPhaseIssued;
                if (_pendingPrefetchAddressPhaseIssued)
                {
                    issuedAddressPhaseEarliestCycle = Math.Max(
                        issuedAddressPhaseEarliestCycle,
                        _pendingPrefetchEarliestCycle);
                }
                // An explicit MOVE or admitted retirement fetch can require a
                // successor previously left cancellable. Commit only
                // its ownership: the original phase floor and context still
                // describe the same request, not a newly issued fetch.
                publicationPhase = commitPendingSuccessor &&
                    _pendingPrefetchPublicationPhase ==
                        M68kInstructionFetchPublicationPhase.CancellableSuccessor
                            ? publicationPhase
                            : _pendingPrefetchPublicationPhase;
                retirementFloor = _pendingPrefetchRetirementFloor;
                publicationContext = _pendingPrefetchPublicationContext;
                _hasPendingPrefetch = false;
                _pendingPrefetchAddress = 0;
                _pendingPrefetchEarliestCycle = 0;
                _pendingPrefetchPublicationPhase = M68kInstructionFetchPublicationPhase.Required;
                _pendingPrefetchRetirementFloor = long.MinValue;
                _pendingPrefetchPublicationContext = default;
                _pendingPrefetchVirtualRequestedCycle = 0;
                _pendingPrefetchBlocksInstructionEntry = false;
                _pendingPrefetchAddressPhaseIssued = false;
            }
            ...
            if (slot == 0)
            {
                ...
                _prefetchCompletedCycle0 = completedCycle;
                _prefetchTimingToken0 = timingToken;
                _prefetchDeferredCpuBusBatchEligible0 = deferredEligible;
            }
            else
            {
                _prefetchWord1 = value;
                _prefetchCompletedCycle1 = completedCycle;
                _prefetchTimingToken1 = timingToken;
        ...
        private void TopUpPrefetchAtRetirement()
        {
            ...
            // An empty entry queue is refilled as two distinct micro-operations:
            // select the next opcode and poll IPL, then commit IRC.  Keep the
            // second transfer in the physical retirement tail without letting
            // its later completion replace the opcode's interrupt sample.
            var sampleAfterFirstRefill =
                _prefetchCount == 0 &&
                _instructionInterruptSampleCycle == long.MinValue;

            // Deferred entry now follows the scalar queue shape. Preserve the
            // captured bus phase even when the opcode required an empty-queue
            // refill; the instruction start is not that transfer boundary.
            var retirementPublicationContext = CaptureInstructionFetchPublicationContext();
            while (_prefetchCount < 2)
            {
				...
				var earliestCycle = _hasPendingPrefetch
					? Math.Max(_pendingPrefetchEarliestCycle, _cpuBusCycle)
					: Math.Max(
						State.Cycles + busCycleStartDelay,
						_cpuBusCycle);
				// A pending transfer remains cancellable until its address phase is
				// explicitly recorded as issued. A bus cursor describes eligibility;
				// subtracting the start delay from it does not prove that microcode
				// started the request. An issued phase, however, remains committed even
				// when contention moves completion past the retirement boundary.
				if (!_pendingPrefetchAddressPhaseIssued &&
					earliestCycle + 2 > _instructionCycleFloor)
                {
                    if (!_hasPendingPrefetch)
                    {
                        _hasPendingPrefetch = true;
                        ...
                        _pendingPrefetchPublicationPhase =
                            M68kInstructionFetchPublicationPhase.CancellableSuccessor;
                        _pendingPrefetchRetirementFloor = _instructionCycleFloor;
                        _pendingPrefetchPublicationContext =
                            retirementPublicationContext;
                        _pendingPrefetchVirtualRequestedCycle = earliestCycle;
                        _pendingPrefetchBlocksInstructionEntry = false;
                        _pendingPrefetchAddressPhaseIssued = false;
                    }
                    return;
                ...
                _cpuRetireBusCycle = Math.Max(_cpuRetireBusCycle, readyCycle);
                if (sampleAfterFirstRefill && _prefetchCount == 1)
                {
                    _instructionInterruptSample = new M68000InterruptSample(
                        readyCycle,
                        _prefetchTimingToken0);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void CommitMovemRetirementPrefetchIfRequestPhaseStarted()
        {
            if (_m68000BusCycleTiming == null ||
                _prefetchCount >= 2 ||
                _prefetchAddress != State.ProgramCounter)
            ...
            var earliestCycle = _hasPendingPrefetch
                ? Math.Max(_pendingPrefetchEarliestCycle, _cpuBusCycle)
                : Math.Max(
                    State.Cycles + _m68000BusCycleTiming.M68000BusCycleStartDelay,
                    _cpuBusCycle);
            var physicalRequestCycle = earliestCycle -
                (_deferredCpuInstructionTiming?.HasPhysicalCpuBusCursor != false
                    ? _m68000BusCycleTiming.M68000BusCycleStartDelay
                    : 0);
            if (physicalRequestCycle >= _instructionCycleFloor ||
                earliestCycle + 2 <= _instructionCycleFloor)
            {
                return;
            }
        ...
        private void PrefetchNextOpcodeBeforeMoveMemoryWriteback()
        {
            ...
            // A 68000 MOVE with a destination extension commits the
            // next opcode before memory writeback, but not the word after it.
            // This order is independent of which instruction filled IR/IRC;
            // a preceding expired DBcc does not move the write ahead of it.
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void CommitStartedConditionalBranchFallthroughPrefetch()
        {
            if (_m68000BusCycleTiming == null ||
                !_hasPendingPrefetch ||
                !_pendingPrefetchAddressPhaseIssued ||
            ...
            // The predecessor selected this fall-through word before Bcc
            // redirected the queue. Once its address phase has begun, the
            // transfer belongs to the physical bus even though the fetched
            // value will be discarded. Promote only that existing request;
            // future look-ahead work remains cancellable.
            var pendingInternalCycles = _pendingInternalCycles;
            _pendingInternalCycles = 0;
            try
            {
                var readyCycle = TopUpPrefetchOne(
                    ...
                    commitPendingSuccessor: true);
                _cpuRetireBusCycle = Math.Max(_cpuRetireBusCycle, readyCycle);
                // The branch redirect starts from the frozen completion edge of
                // the already-started fall-through transfer.  Keeping the old
                // architectural cursor here would let the target address phase
                // overlap that transfer and make it eligible one chip slot early.
                State.Cycles = Math.Max(State.Cycles, readyCycle);
            }
            finally
            {
                _pendingInternalCycles = pendingInternalCycles;
            }
        }
        ...
        private void CommitShortConditionalBranchFallthroughPrefetch()
        {
            ...
            {
                _prefetchAddress = State.ProgramCounter;
                var selectedReadyCycle = TopUpPrefetchOne(
                    out _,
                    M68kInstructionFetchPublicationPhase.Required,
                    long.MinValue,
                    CaptureInstructionFetchPublicationContext(),
                    commitPendingSuccessor: false);
                _cpuRetireBusCycle = Math.Max(
                    _cpuRetireBusCycle,
                    selectedReadyCycle);
            }

            SampleM68000IplAtShortConditionalBranchControlPhase();
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void SampleM68000IplAtShortConditionalBranchControlPhase()
        {
            if (_pendingInternalCycles != 0)
            {
                State.Cycles += _pendingInternalCycles;
            ...
            var selectedReadyCycle = _prefetchCompletedCycle0;
            State.Cycles = Math.Max(State.Cycles, selectedReadyCycle);
            _cpuRetireBusCycle = Math.Max(
                _cpuRetireBusCycle,
                selectedReadyCycle);
            var timingToken = State.Cycles == selectedReadyCycle
                ? _prefetchTimingToken0
                : 0;
            _instructionInterruptSample = new M68000InterruptSample(
                State.Cycles,
        ...
        private void PrefetchFallthroughAfterMoveSourceRead(
            bool commitPendingSuccessor = true,
            bool sampleIplAfterDataPhase = false)
            => CommitRequiredSequentialFinalPrefetch(
                commitPendingSuccessor,
                sampleIplAfterDataPhase);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void CommitRequiredSequentialFinalPrefetch(
            bool commitPendingSuccessor = true,
            bool sampleIplBeforeFetch = false,
            bool requireTransferRetirementEdge = false)
        {
            if ((State.ProgramCounter & 1) != 0)
            {
                return;
            ...
            if (_prefetchAddress == State.ProgramCounter)
            {
                // Some MC68000 microsequences poll IPL at the completed data
                // phase before issuing their mandatory final queue refill.
                // Keep that sample attached to the earlier physical phase;
                // completion of the following fetch must not move it.
                if (sampleIplBeforeFetch)
                {
                    SampleM68000IplAtCurrentBusPhase();
                }
                var readyCycle = TopUpPrefetchOne(
                ...
                // Some final reads may overlap retirement after publication;
                // others are synchronous micro-operations whose data phase is
                // part of the instruction itself. Fast-memory publication can
                // precede that physical data edge, while NextBusCycle retains
                // the complete bus tail. Chip transfers normally make the two
                // edges identical.
                var transferRetireCycle = readyCycle;
                if (requireTransferRetirementEdge)
                {
                    var startDelay =
                        _m68000BusCycleTiming?.M68000BusCycleStartDelay ?? 0;
                    transferRetireCycle = Math.Max(
                        transferRetireCycle,
                        _cpuBusCycle - startDelay);
                }
                _cpuRetireBusCycle = Math.Max(
                    _cpuRetireBusCycle,
                    transferRetireCycle);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void SampleM68000IplAtCurrentBusPhase()
        {
            if (_useM68020BriefIndexedAddressing ||
                _instructionInterruptSampleCycle != long.MinValue)
            {
            ...
            _instructionInterruptSample = new M68000InterruptSample(
                _cpuBusReadyCycle,
                _cpuBusReadyTimingToken);
            RegisterDeferredInterruptSamples();
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void SampleM68000IplAtQueuedOpcode()
        {
            if (_useM68020BriefIndexedAddressing ||
                _instructionInterruptSampleCycle != long.MinValue ||
                _prefetchCount == 0 ||
            ...
            // Register-immediate microcode selects the following opcode and
            // polls IPL before the final IRC refill. Preserve the selected
            // word's physical identity so deferred replay cannot move the
            // sample to that later tail transfer.
            _instructionInterruptSample = new M68000InterruptSample(
                _prefetchCompletedCycle0,
                _prefetchTimingToken0);
            RegisterDeferredInterruptSamples();
            ...
            long issuedAddressPhaseEarliestCycle = 0)
        {
            var busAddress = GetCpuBusAddress(address);
            var cycle = addressPhaseAlreadyIssued
                ? Math.Max(issuedAddressPhaseEarliestCycle, _cpuBusCycle)
                : BeginPrefetchBusAccessCycle();
            requestedCycle = cycle;
            var value = ReadInstructionFetchWord(
                busAddress,
                ref cycle,
                out deferredEligible,
                publicationPhase,
            ...
            var timing = GetM68000BusAccessTiming(
                ...
                requestedCycle,
                cycle);
            _cpuBusCycle = timing.NextBusCycle;
            _cpuBusReadyCycle = timing.ReadyCycle;
            if (_deferredCpuBusBatchExecutionActive && deferredEligible)
            {
                _cpuBusReadyTimingToken = _deferredCpuInstructionTiming?
                    .CaptureDeferredCpuInstructionFetchTimingToken() ?? 0;
            }
            completedCycle = timing.ReadyCycle;
            if (_cpuBusPhaseTrace != null)
        ...
        private ushort ReadInstructionFetchWord(
            uint address,
            ref long cycle,
            out bool deferredEligible,
            M68kInstructionFetchPublicationPhase publicationPhase = M68kInstructionFetchPublicationPhase.Required,
            ...
            in M68kInstructionFetchPublicationContext publicationContext = default)
        {
            if (_instructionFetchWindowBus != null)
            {
                ...
                deferredEligible =
                    _deferredCpuInstructionTiming?.IsDeferredCpuBusBatchEligibleInstructionFetchWindow(
                        in _instructionFetchWindow) == true;
                return bus.ReadInstructionFetchWindowWord(
        ...
        private byte ReadByte(uint address)
        {
            ...
            var cycle = BeginCpuBusAccessCycle();
            var requestedCycle = cycle;
            var value = TCpuDataAccess.ReadByte(_typedBus, busAddress, ref cycle);
            CompleteRequiredCpuBusAccess(
                busAddress,
        ...
        private ushort ReadWord(uint address)
        {
            ...
            var cycle = BeginCpuBusAccessCycle();
            var requestedCycle = cycle;
            var value = TCpuDataAccess.ReadWord(_typedBus, busAddress, ref cycle);
            CompleteRequiredCpuBusAccess(
                busAddress,
                M68kOperandSize.Word,
                M68kBusAccessKind.CpuDataRead,
        ...
        protected uint ReadLong(uint address)
        {
            ...
            var cycle = BeginCpuBusAccessCycle();
            var requestedCycle = cycle;
            var value = TCpuDataAccess.ReadLong(_typedBus, busAddress, ref cycle);
            CompleteRequiredCpuBusAccess(
                busAddress,
                M68kOperandSize.Long,
                M68kBusAccessKind.CpuDataRead,
        ...
        private void WriteByte(uint address, byte value)
        {
            ...
            var cycle = BeginCpuBusAccessCycle();
            var requestedCycle = cycle;
            TCpuDataAccess.WriteByte(_typedBus, busAddress, value, ref cycle);
            CompleteRequiredCpuBusAccess(
                busAddress,
                M68kOperandSize.Byte,
                M68kBusAccessKind.CpuDataWrite,
        ...
        private void WriteTasByte(uint address, byte value)
        {
            ...
            var cycle = BeginCpuBusAccessCycle();
            var requestedCycle = cycle;
            TCpuDataAccess.WriteTasByte(_typedBus, busAddress, value, ref cycle);
            CompleteRequiredCpuBusAccess(
                busAddress,
                M68kOperandSize.Byte,
                M68kBusAccessKind.CpuDataWrite,
        ...
        private void WriteWord(uint address, ushort value)
        {
            ...
            var cycle = BeginCpuBusAccessCycle();
            var requestedCycle = cycle;
            TCpuDataAccess.WriteWord(_typedBus, busAddress, value, ref cycle);
            CompleteRequiredCpuBusAccess(
                busAddress,
                M68kOperandSize.Word,
                M68kBusAccessKind.CpuDataWrite,
        ...
        private void WriteLong(uint address, uint value)
        {
            ...
            var cycle = BeginCpuBusAccessCycle();
            var requestedCycle = cycle;
            TCpuDataAccess.WriteLong(_typedBus, busAddress, value, ref cycle);
            CompleteRequiredCpuBusAccess(
                busAddress,
                M68kOperandSize.Long,
                M68kBusAccessKind.CpuDataWrite,
        ...
        private void CommitIssuedPendingPrefetch()
        {
            if (!_hasPendingPrefetch ||
                !_pendingPrefetchAddressPhaseIssued)
            {
            ...
            var queueTailAddress = unchecked(
                _prefetchAddress + (uint)(_prefetchCount * 2));
            if (_prefetchCount < 2 &&
                _pendingPrefetchAddress == queueTailAddress)
            {
                var readyCycle = TopUpPrefetchOne(
                    out _,
                    M68kInstructionFetchPublicationPhase.Required,
            ...
            // A redirect or another CPU bus claimant cannot cancel an address
            // phase that has already started. Complete that physical transfer
            // from its frozen earliest grant, but discard the word when the
            // two-entry queue has no matching destination.
            var address = _pendingPrefetchAddress;
            var earliestCycle = _pendingPrefetchEarliestCycle;
            var publicationPhase = _pendingPrefetchPublicationPhase;
            var retirementFloor = _pendingPrefetchRetirementFloor;
            var publicationContext = _pendingPrefetchPublicationContext;
            _hasPendingPrefetch = false;
            _pendingPrefetchAddress = 0;
            _pendingPrefetchEarliestCycle = 0;
            _pendingPrefetchPublicationPhase =
            ...
            _ = ReadPrefetchWord(
                ...
                retirementFloor,
                in publicationContext,
                addressPhaseAlreadyIssued: true,
                issuedAddressPhaseEarliestCycle: earliestCycle);
            _cpuRetireBusCycle = Math.Max(
                _cpuRetireBusCycle,
                completedCycle);
        }
        ...
        private long BeginCpuBusAccessCycle()
        {
            ...
            return cycle < _cpuBusCycle ? _cpuBusCycle : cycle;
        }
        ...
        private void CompleteCpuBusAccess(long completedCycle)
        {
            ...
            _cpuBusReadyCycle = completedCycle;
            if (_cpuRetireBusCycle < completedCycle)
            {
                _cpuRetireBusCycle = completedCycle;
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void CompleteRequiredCpuBusAccess(
            uint address,
            M68kOperandSize size,
            M68kBusAccessKind accessKind,
            bool isWrite,
            ...
            long completedCycle)
        {
            ...
            _cpuBusCycle = timing.NextBusCycle;
            _cpuBusReadyCycle = timing.ReadyCycle;
            if (_cpuRetireBusCycle < timing.ReadyCycle)
            {
                _cpuRetireBusCycle = timing.ReadyCycle;
            }

            State.Cycles = timing.ReadyCycle;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private M68000BusAccessTiming GetM68000BusAccessTiming(
            uint address,
            M68kOperandSize size,
            M68kBusAccessKind accessKind,
            bool isWrite,
            long requestedCycle,
            long completedCycle)
            => _m68000BusCycleTiming?.GetM68000BusAccessTiming(
                address,
                size,
                accessKind,
                isWrite,
                requestedCycle,
                completedCycle) ??
                new M68000BusAccessTiming(completedCycle, completedCycle);
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
            ...
            bool isWrite)
        {
            ...
            if (_m68000BusCycleTiming != null &&
                accessKind != M68kBusAccessKind.CpuInterruptAcknowledge)
            {
                requestedCycle -= _m68000BusCycleTiming.M68000BusCycleStartDelay;
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void AddPendingInternalCycles(int cycles)
        {
            System.Diagnostics.Debug.Assert(cycles >= 0, "MC68000 pending cycle increments must be non-negative.");
            _pendingInternalCycles += cycles;
        }

        private void AddInternalNoBusInstructionCycles(
            int instructionCycles,
            int internalCycles,
            M68kInternalNoBusWindowKind kind)
        {
            AddInstructionCycles(instructionCycles);
            if (internalCycles <= 0 ||
                _deferredCpuInstructionTiming is not { IsInternalNoBusWindowEnabled: true })
            {
                return;
            }

            var currentCycle = Math.Max(State.Cycles, _cpuRetireBusCycle);
            var availableNoBusCycles = (int)Math.Min(
                int.MaxValue,
                Math.Max(0, (_instructionCycleStart + instructionCycles) - currentCycle - EstimatePrefetchTopUpCycles()));
            var latestNoBusCycles = Math.Min(
                internalCycles,
                availableNoBusCycles);
            if (latestNoBusCycles <= 0)
            {
                return;
            }

            if (_deferredCpuInstructionTiming.TryAdvanceInternalNoBusWindow(
                State,
                currentCycle,
                latestNoBusCycles,
                kind))
            {
                AddPendingInternalCycles(latestNoBusCycles);
            }
        }

        private int EstimatePrefetchTopUpCycles()
        {
            if ((State.ProgramCounter & 1) != 0)
            {
                return 0;
            ...
            return _prefetchAddress == State.ProgramCounter
                ? (2 - _prefetchCount) * 4
                : 0;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void AllowInstructionRetireAtCycleFloor()
        {
            System.Diagnostics.Debug.Assert(
                _instructionCycleFloorActive,
                "Instruction retire overlap may only be applied while an instruction cycle floor is active.");
            if (_cpuRetireBusCycle > _instructionCycleFloor)
            {
                _cpuRetireBusCycle = _instructionCycleFloor;
            }
        ...
        private void BeginInstructionCycleFloor(long startCycle)
        {
            ...
            _consumeWithoutPrefetch = false;
            _nextCpuDataAccessDelayCycles = 0;
            _instructionDeferredCpuTimingDependencyToken = 0;
            _instructionDeferredCpuTimingDependencyVirtualReadyCycle = -1;
            _instructionFetchPublicationGroup++;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RegisterDeferredInterruptSamples()
            => RegisterDeferredInterruptSamples(
                _lastInterruptSample.DeferredFetchToken,
                _instructionInterruptSample.DeferredFetchToken);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RegisterDeferredInterruptSamples(in M68000FixedBatchContext context)
            => RegisterDeferredInterruptSamples(
                context.LastInterruptSample.DeferredFetchToken,
                context.InstructionInterruptSample.DeferredFetchToken);

        private void RegisterDeferredInterruptSamples(ulong lastToken, ulong instructionToken)
        {
            var hasSamples = lastToken != 0 || instructionToken != 0;
            if (_deferredCpuInstructionTiming == null ||
                (!hasSamples && !_hasRegisteredDeferredInterruptSamples))
            {
                return;
            }

            _deferredCpuInstructionTiming.RegisterDeferredCpuInterruptSamples(lastToken, instructionToken);
            _hasRegisteredDeferredInterruptSamples = hasSamples;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private M68000InterruptSample ResolveDeferredInterruptSample(M68000InterruptSample sample)
        {
            if (sample.DeferredFetchToken == 0 || _deferredCpuInstructionTiming == null)
            {
                return sample;
            }

            var resolved = _deferredCpuInstructionTiming.GetDeferredCpuInterruptSampleResolution(
                sample.DeferredFetchToken);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void EnsureInstructionInterruptSample()
        {
        ...
        private readonly struct EaOperand
        {
            ...
            public uint ReadTstLongWithIplSampleBetweenWords()
            {
                _cpu._dataAccessStackedProgramCounter = _addressErrorStackedProgramCounter;
                _cpu._dataReadFaultAccessKind = _readFaultAccessKind;
                if (EaCycles > 0)
                ...
                CompleteBeforeReadMemoryAccess();
                var high = _cpu.ReadWord(Address);
                _cpu.SampleM68000IplAtCurrentBusPhase();
                var low = _cpu.ReadWord(Address + 2);
                CompleteReadMemoryAccess();
                return ((uint)high << 16) | low;
            }
    ...
    internal sealed class M68kInterpreter : M68kInterpreterCore<IM68kBus, M68kNoExactCpuDataAccess<IM68kBus>>
    {
        ...
        internal M68kInterpreter(
            ...
            : base(
                ...
                enableOpcodePlan,
                opcodePlanDispatch,
                useM68020BriefIndexedAddressing)
        {
        }
