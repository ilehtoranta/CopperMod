...
if (-not $packageFile.Name.StartsWith("CopperFloat.", [StringComparison]::OrdinalIgnoreCase)) {
    throw "PackagePath must point to a CopperFloat package."
}
...
Invoke-NuGetPush -Path $packageFile.FullName -Label "CopperFloat package"

if (-not $NoSymbols) {
    $symbolPackagePath = [System.IO.Path]::ChangeExtension($packageFile.FullName, ".snupkg")
    if (Test-Path $symbolPackagePath) {
        Invoke-NuGetPush -Path $symbolPackagePath -Label "CopperFloat symbol package"
    }
    else {
