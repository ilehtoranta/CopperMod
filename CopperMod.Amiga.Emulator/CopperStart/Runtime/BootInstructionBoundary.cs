...
internal sealed class BootInstructionBoundaryContext
{
    public BootInstructionBoundaryContext(AmigaBus bus, M68kCpuState cpu, uint dosReturnAddress,
        Func<bool> romBoot, Action activateRomExec, Func<bool> dispatchTasks,
        Func<bool> canExecuteCurrentTask, Func<bool> dispatchInterrupts,
    ...
    { Bus = bus; Cpu = cpu; DosReturnAddress = dosReturnAddress; RomBoot = romBoot; ActivateRomExec = activateRomExec; DispatchTasks = dispatchTasks; CanExecuteCurrentTask = canExecuteCurrentTask; DispatchInterrupts = dispatchInterrupts; EnsureTrapVectors = ensureTrapVectors; EnsureLowMemory = ensureLowMemory; EnsureAutovectors = ensureAutovectors; RecoverTrap = recoverTrap; StartDosContinuation = startDosContinuation; RecordNullPc = recordNullPc; ContinueStartup = continueStartup; SkipBootHeader = skipBootHeader; ApplyLanguage = applyLanguage; BootReadComplete = bootReadComplete; DispatchHardwareInterrupt = dispatchHardwareInterrupt; NextSyntheticBoundary = nextSyntheticBoundary; AdvanceSynthetic = advanceSynthetic; NextHostDeviceBoundary = nextHostDeviceBoundary; }
    public AmigaBus Bus { get; } public M68kCpuState Cpu { get; } public uint DosReturnAddress { get; }
    public Func<bool> RomBoot { get; } public Action ActivateRomExec { get; } public Func<bool> DispatchTasks { get; }
    public Func<bool> CanExecuteCurrentTask { get; } public Func<bool> DispatchInterrupts { get; }
    ...
    IM68kPureCpuTraceBatchBoundary, IM68kBusAccessTraceBatchBoundary, IM68kDeferredCpuBusBatchBoundary
{
    ...
    public bool BeforeInstruction()
    {
		...
		var romBoot = _context.RomBoot();
        if (romBoot) _context.ActivateRomExec();
        if (_context.DispatchTasks()) return true;
        if (!_context.CanExecuteCurrentTask()) return false;
        if (romBoot && _context.DispatchInterrupts()) return true;
        if (!romBoot) { _context.EnsureTrapVectors(); _context.EnsureLowMemory(); _context.EnsureAutovectors(); }
		var decision = PortableRuntime.BootBoundaryCore.DecideBefore(false, romBoot, _context.Cpu.ProgramCounter, _instructions, _context.DosReturnAddress);
		if (decision == PortableRuntime.BootBoundaryDisposition.TryRecovery) { if (_context.RecoverTrap() || _context.StartDosContinuation()) return true; Completed = true; return false; }
		if (decision == PortableRuntime.BootBoundaryDisposition.RecordNullPcAndComplete) { _context.RecordNullPc(); Completed = true; return false; }
		if (decision == PortableRuntime.BootBoundaryDisposition.HandleDosReturn) return HandleDosReturn();
	...
	private void AfterBatch(long previousCycle, long currentCycle, int count) { if (count <= 0) return; _schedule.AdvanceThrough(previousCycle, currentCycle); var mask = (_context.Cpu.StatusRegister >> 8) & 7; _context.Bus.AdvanceHardwareEventsTo(currentCycle, mask); _context.DispatchHardwareInterrupt(); _instructions += count; if (PortableRuntime.BootBoundaryCore.CompleteAfterBatch(_context.BootReadComplete(), _runMode == AmigaBootRunMode.StopAfterBootDiskRead)) Completed = true; }
	public bool TryFastForwardStoppedInstruction(M68kCpuState state, long targetCycle, out long advancedCycles) { advancedCycles = 0; if (!BeforeInstruction() || targetCycle <= state.Cycles) return false; var previous = state.Cycles; targetCycle = _schedule.GetNextBoundaryCycle(previous, targetCycle); var mask = (state.StatusRegister >> 8) & 7; var wake = _context.Bus.GetNextCpuBatchWakeCandidateCycle(previous, targetCycle, mask); wake = PortableRuntime.ExecutionBoundaryCore.ClampProgress(previous, targetCycle, wake); advancedCycles = wake - previous; state.Cycles = wake; AfterInstruction(previous, wake); return true; }
}
