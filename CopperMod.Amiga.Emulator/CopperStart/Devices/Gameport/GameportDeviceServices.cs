...
internal sealed class GameportDeviceServices : IDisposable
{
    ...
    private readonly Queue<RawMouseEvent>[] _events = { new(), new() };
    ...
    private readonly record struct RawMouseEvent(byte Code, short X, short Y, ushort Qualifiers);
    ...
    public long GetNextDeadline(long currentCycle, long targetCycle)
    {
        ...
        for (var unit = 0; unit < 2; unit++) if (_nextTimeout[unit] > currentCycle && _nextTimeout[unit] < result) result = _nextTimeout[unit];
        return result;
    }
    ...
    public void ProcessPending(M68kCpuState state)
    {
        ...
        for (var unit = 0; unit < 2; unit++)
        {
            ...
            if (_events[unit].Count == 0 && state.Cycles >= _nextTimeout[unit]) { _events[unit].Enqueue(new RawMouseEvent(IeCodeNoButton, 0, 0, _qualifiers[unit])); _nextTimeout[unit] = long.MaxValue; }
            ...
            foreach (var entry in new List<PendingRead>(_pending.Values))
            {
                ...
                if (WriteMouseEvent(entry.Request, unit, state.Cycles)) _pending.Remove(entry.Request);
            }
        }
    }
    ...
    public void Dispose()
    {
        ...
        _gateways.Clear(); _units.Clear(); _pending.Clear(); Array.Clear(_controllerTypes); Array.Clear(_lastJoy); Array.Clear(_lastPrimary); Array.Clear(_lastSecondary); Array.Clear(_qualifiers); foreach (var events in _events) events.Clear(); _nextTimeout[0] = _nextTimeout[1] = long.MaxValue; _beginIoAddress = 0; _beginIoToken = 0; _forwardingBeginIo = false; DeviceBase = 0;
    }
    ...
    private void BeginIo(M68kCpuState state)
    {
        ...
        switch (command)
        {
            ...
            case GpdReadEvent:
                ...
                if (_events[unit].Count != 0) WriteMouseEvent(request, unit, state.Cycles); else { _pending[request] = new PendingRead(request, unit); ScheduleTimeout(unit, state.Cycles); }
                break;
            default:
    ...
    private void EnqueueMotion(int unit, int x, int y)
    {
        ...
        _events[unit].Enqueue(new RawMouseEvent(IeCodeNoButton, (short)x, (short)y, (ushort)(_qualifiers[unit] | 0x8000)));
    }

    private void ScheduleTimeout(int unit, long cycle)
    {
        var ticks = (_triggers[unit][2] << 8) | _triggers[unit][3];
        _nextTimeout[unit] = ticks == 0 ? long.MaxValue : cycle + (long)ticks * _bus.RasterTiming.GetFrameCycles(_bus.RasterTiming.LongFrameLines);
    }
    ...
    private void EnqueueButton(int unit, byte code, ushort qualifier, bool down)
    {
        ...
        _events[unit].Enqueue(new RawMouseEvent((byte)(down ? code : code | IeCodeUpPrefix), 0, 0, _qualifiers[unit]));
    }

    private bool WriteMouseEvent(uint request, int unit, long cycle)
    {
